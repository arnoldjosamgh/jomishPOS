const BASE_URL = (window.location.protocol === 'file:' || window.location.hostname.includes('github.io')) ? 'https://jomish-business-suite.onrender.com' : '';
window.API_URL = `${BASE_URL}/api`;
const API_URL = window.API_URL;
console.log('JOMISH APP V2 LOADED');
let USER_ROLE = '';
let USER_NAME = '';
window.USER_PERMISSIONS = { can_see_dashboard: 0, can_see_hr: 0, can_see_attendance: 1, can_see_sme: 0, can_see_pos: 0, can_see_secretary: 0, can_see_transport: 0 };
let USER_PERMISSIONS = window.USER_PERMISSIONS;
let ENABLED_TABS = { dashboard: true, hr: true, attendance: true, sme: true, pos: true, secretary: true, transport: true, schedules: true, hardware: true, system_users: true };
let barcodeBuffer = "";
let lastKeyTime = Date.now();
let soldBarcodes = new Set();
let isPrinting = false;
let GLOBAL_SHIFTS = [
    { id: 'morning', label: 'Morning', icon: 'fa-solid fa-sun', color: '#F59E0B', start_time: '08:00', end_time: '16:00' },
    { id: 'afternoon', label: 'Afternoon', icon: 'fa-solid fa-cloud-sun', color: '#6366F1', start_time: '16:00', end_time: '23:59' }
];
...
function openCalModal() {
    ...
    if (!m) { alert('ERROR: Modal element #cal-event-modal not found!'); return; }
...
function formatDisplayDate(dateStr, showTime = true) {
    ...
    if (!s || s === 'null' || s === 'undefined' || s === 'CURRENT_DATETIME') return '—';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    } catch (e) { console.error('Permission parse err', e); }
    ...
    const _uip = document.querySelector('.user-info p'); if(_uip) _uip.innerText = `Logged in as: ${localStorage.getItem('jomish_name')} (${USER_ROLE})`;
    // JomishPOS topbar
    const _tbUser = document.getElementById('pos-topbar-username');
    if (_tbUser) _tbUser.textContent = `${localStorage.getItem('jomish_name') || 'User'} (${USER_ROLE})`;
    initNavigation();
    enforceRBAC();
    autoRedirect();
    ...
    if (skeleton) {
        ...
        setTimeout(() => { if (skeleton.parentNode) skeleton.remove(); }, 450);
    }
    ...
    setTimeout(() => {
        ...
        if (ipInput && savedIp) { ipInput.value = savedIp; updateNetworkPrinterStatus(savedIp); }
    ...
    console.log('[CAL] btn found:', !!_calBtn, '| modal found:', !!_calModal);
    ...
    if (_calBtn && _calModal) {
        _calBtn.addEventListener('click', () => {
            ...
            _calModal.style.setProperty('background-color', 'rgba(0,0,0,0.7)', 'important');
        ...
        console.error('[CAL ERROR] Could not find: btn=' + !!_calBtn + ' modal=' + !!_calModal);
    }
    ...
    document.getElementById('form-add-tx')?.addEventListener('submit', handleAddTx);

    // Attendance Scan Simulation
    document.getElementById('btn-simulate-scan')?.addEventListener('click', handleScan);
    ...
    document.getElementById('btn-auto-schedule')?.addEventListener('click', handleAutoSchedule);
    ...
    if (btnDownloadAtt) btnDownloadAtt.addEventListener('click', downloadAttendancePDF);

    // User Management Modal Handlers
    const btnClosePassModal = document.getElementById('btn-close-pass-modal');
    if (btnClosePassModal) btnClosePassModal.addEventListener('click', () => document.getElementById('set-password-modal').classList.add('hidden'));
    ...
    if (btnCloseEditProd) btnCloseEditProd.addEventListener('click', () => {
    ...
    if (btnAddProd) btnAddProd.addEventListener('click', () => document.getElementById('add-product-modal').classList.remove('hidden'));
    
    const btnCloseProd = document.getElementById('btn-close-prod-modal');
    if (btnCloseProd) btnCloseProd.addEventListener('click', () => document.getElementById('add-product-modal').classList.add('hidden'));
    ...
    if (btnGenerateBulk) btnGenerateBulk.addEventListener('click', () => window.generateBulkBarcodes());
    
    const btnIgnoreBarcodes = document.getElementById('btn-ignore-barcodes');
    if (btnIgnoreBarcodes) btnIgnoreBarcodes.addEventListener('click', () => window.ignoreRemainingBarcodes());

    const btnCloseScan = document.getElementById('btn-close-scan-session');
    if (btnCloseScan) btnCloseScan.addEventListener('click', () => window.closeScanSession());

    // POS Search
    const posSearch = document.getElementById('pos-search');
    if (posSearch) posSearch.addEventListener('input', (e) => loadPOSProducts(e.target.value));

    // SME Finance Search
    const smeSearch = document.getElementById('sme-search');
    if (smeSearch) smeSearch.addEventListener('input', (e) => loadTransactions(e.target.value));
    ...
    if (btnCheckout) {
        btnCheckout.addEventListener('click', () => {
            ...
            document.getElementById('pay-total').textContent = 'UGX ' + Math.round(total).toLocaleString();
...
function switchHRView(viewName) {
    ...
    document.querySelectorAll('.hr-sub-view').forEach(v => v.classList.add('hidden'));
    
    // UI Toggle for buttons
    const hrBtns = document.querySelectorAll('.tab-scroller .nav-btn');
    hrBtns.forEach(b => {
        b.classList.remove('active');
        if (b.getAttribute('id').includes(viewName)) b.classList.add('active');
    });
    ...
    if (viewName === 'recruitment') {
        ...
        if (!document.getElementById('payroll-month').value) {
            const now = new Date();
            document.getElementById('payroll-month').value = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
        }
        loadPayrollStatus();
...
async function loadEmployeePermissions() {
    ...
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;padding:20px;"><i class="fa-solid fa-spinner fa-spin"></i> Loading...</td></tr>';
    try {
        const res = await fetchAuth(`${API_URL}/employees`);
        const data = await res.json();
        const employees = data.employees || [];
        if (employees.length === 0) {
            tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;color:var(--text-muted);">No employees found.</td></tr>';
            return;
        }
        tbody.innerHTML = '';
        const PERMS = [
            ...
            'can_see_hardware','can_see_system_users','can_see_schedules'
        ];
        ...
        tbody.innerHTML = '<tr><td colspan="13" style="color:red;text-align:center;">Failed to load employees.</td></tr>';
...
async function updateEmployeePermission(empId, permKey, isChecked, checkboxEl) {
    ...
    try {
        ...
        showToast(`✅ Permission updated. Ask them to re-login to see changes.`, 'success');
    } catch(e) {
...
async function loadPOSExpenses() {
    ...
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:30px; color:#94A3B8;">Loading...</td></tr>';
    ...
    try {
        ...
        const todayEAT = new Date(Date.now() + EAT_OFFSET_MS).toISOString().split('T')[0];

        let todayTotal = 0, allTotal = 0;
        expenses.forEach(t => {
            const amt = parseFloat(t.amount) || 0;
            allTotal += amt;
            const txDate = new Date(String(t.transaction_date).replace(' ', 'T') + (t.transaction_date.includes('Z') ? '' : 'Z'));
            const txDateEAT = new Date(txDate.getTime() + EAT_OFFSET_MS).toISOString().split('T')[0];
        ...
        if (expenses.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:40px; color:#94A3B8;">No expenses recorded yet.</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:30px; color:#EF4444;">Failed to load expenses.</td></tr>';
    }
}
...
async function handlePOSExpenseSubmit(e) {
    ...
    if (!amount || amount <= 0) {
        ...
        if (btn) { btn.disabled = false; btn.textContent = 'Record Expense'; }
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            showToast(`Expense of UGX ${amount.toLocaleString()} recorded.`, 'success');
            e.target.reset();
            loadPOSExpenses();
...
async function saveBusinessConfig() {
    const config = {
        days: Array.from(document.querySelectorAll('#config-days input:checked')).map(i => i.value),
...
async function loadBusinessConfig() {
    try {
        ...
        const staffingContainer = document.getElementById('config-staffing-dynamic');
        ...
        if (document.getElementById('config-shift-hours')) document.getElementById('config-shift-hours').value = config.hoursPerShift || 8;
        if (document.getElementById('config-open-time')) document.getElementById('config-open-time').value = config.openingTime || '08:00';

        // Populate dynamic staffing counts
        document.querySelectorAll('.staff-req').forEach(i => {
            const role = i.getAttribute('data-role');
            if (config.staffing && config.staffing[role]) i.value = config.staffing[role];
        });
...
async function addNewRoleBlueprint() {
    ...
    } catch (e) { console.error('Role Add Error:', e); }
}

async function deleteRoleBlueprint(roleName) {
    if (!confirm(`Delete role "${roleName}"? This will affect all staff assigned to this role.`)) return;
    try {
        const res = await fetchAuth(`${API_URL}/roles/${roleName}`, { method: 'DELETE' });
        if (res.ok) {
            loadBusinessConfig();
            loadRoles();
        }
...
async function loadUpcomingPay() {
    try {
        const res = await fetch(`${API_URL}/employees/upcoming-pay`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('jomish_token')}` } });
        if (res.ok) {
            const data = await res.json();
            ...
            if (data.length > 0) {
                ...
                if (itemsDiv) itemsDiv.innerHTML = data.map(e => `<div style="background: white; padding: 10px; border-radius: 8px; border: 1px solid #E5E7EB; flex: 1 1 200px;">
                    <div style="font-weight: bold; color: #111827;">${e.first_name} ${e.last_name}</div>
                    <div style="font-size: 0.8rem; color: #6B7280;">${e.role} - Pay due on ${new Date(e.next_pay_date).toLocaleDateString()}</div>
                </div>`).join('');
            } else {
                if (panel) panel.classList.add('hidden');
            }
        }
    } catch (e) { console.error('Upcoming pay error:', e); }
}
...
async function loadPresenceSummary() {
    ...
    } catch (e) { console.error('Presence intelligence failure', e); }
}
...
function registerDevice() {
    ...
    if (!devId) {
        devId = 'JOM-DEV-' + Math.random().toString(36).substring(2, 9).toUpperCase();
        localStorage.setItem('jomish_device_id', devId);
    }

    const devName = navigator.platform + ' (' + (navigator.vendor || 'Generic') + ')';
    const devType = /Mobile|Android|iPhone/i.test(navigator.userAgent) ? 'MOBILE' : 'TERMINAL';
...
async function loadDevices() {
    ...
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:#94A3B8;">Loading devices...</td></tr>';
    ...
    try {
        ...
        if (!data.devices || data.devices.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:#94A3B8;">No devices connected.</td></tr>';
            return;
        }
        ...
        data.devices.forEach(d => {
            ...
            const lastSeenStr = lastSeen
                ? lastSeen.toLocaleString('en-UG', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit', second:'2-digit' })
                : 'Never';
            ...
            const isMe = d.device_id === myDevId ? ' <b style="color:var(--primary);">(This Device)</b>' : '';
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--danger);">Error loading devices</td></tr>';
    }
}
...
async function loadFinanceIntelligence() {
    ...
    // Only CEO and HR can see Financial Intelligence
    ...
    try {
        ...
        const updateCard = (prefix, values) => {
            ...
            if (incomeEl)  incomeEl.innerText  = `UGX ${(values.income  || 0).toLocaleString()}`;
            if (expenseEl) expenseEl.innerText = `UGX ${(values.expense || 0).toLocaleString()}`;
            if (profitEl) {
                profitEl.innerText   = `UGX ${(values.profit || 0).toLocaleString()}`;
                profitEl.style.color = (values.profit || 0) >= 0 ? '#10B981' : '#EF4444';
            }
        };
        ...
        if (data._debug) {
            console.log('Finance Summary Debug:', data._debug);
        }

    } catch (e) { console.error('Finance Intel Error:', e); }
}
...
async function loadLowStockAlerts() {
    try {
        ...
        data.alerts.forEach(p => {
            ...
            badge.style = "background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2); padding: 5px 12px; border-radius: 6px; font-size: 0.8rem; display: flex; align-items: center; gap: 8px;";
            badge.innerHTML = `
...
async function pingDevice(device_id, device_name, device_type) {
    try {
        ...
        await fetch(`${API_URL}/devices/ping`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ device_id, device_name, device_type, company_schema, client_ip })
        });
    } catch (e) { console.error('Radar Ping Error:', e); }
}
...
async function loadHardwareRadar() {
    ...
    try {
        ...
        if (!data.devices || data.devices.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">No hardware detected yet.</td></tr>';
            return;
        }
        ...
        data.devices.forEach(d => {
            ...
            const lastActiveStr = lastActive
                ? lastActive.toLocaleString('en-UG', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit', second:'2-digit' })
                : 'Never';
...
function startLiveClock() {
    ...
    window._liveclockInterval = setInterval(() => {
...
function initSockets() {
    if (typeof io === 'undefined') {
        ...
        if (statusEl) {
            ...
            statusEl.innerHTML = '<i class="fa-solid fa-circle" style="color: #EF4444;"></i> Offline Mode (No Sync)';
        }
        return;
    }
    ...
    socket.on('connect', () => {
        ...
        if (statusEl) {
            ...
            statusEl.innerHTML = '<i class="fa-solid fa-circle" style="color: #10B981;"></i> Live Sync Active';
        }
    });
    ...
    socket.on('disconnect', () => {
        ...
        if (statusEl) {
            statusEl.style.color = 'var(--danger)';
            statusEl.innerHTML = '<i class="fa-solid fa-circle" style="color: #EF4444;"></i> Disconnected';
        }
    });
    ...
    socket.on('db_updated', (data) => {
        if (data.module === 'employees') {
            ...
            if (USER_PERMISSIONS.can_see_attendance) { loadAttendance(); loadDashboard(); }
        } else if (data.module === 'transactions') {
            if (USER_PERMISSIONS.can_see_sme) { loadTransactions(); loadDashboard(); }
        } else if (data.module === 'schedules' || data.module === 'shift_assignments') {
            // Refresh dashboard timetable always; full grid only if tab is active
            if (typeof window.loadDayTimetable === 'function') window.loadDayTimetable();
            const section = document.getElementById('schedules');
            if (section && section.classList.contains('active') && typeof window.loadShiftTimetable === 'function') {
            ...
            if (USER_PERMISSIONS.can_see_pos && !window._skipProductReload) loadPOSProducts();
            ...
            if (USER_PERMISSIONS.can_see_secretary) {
                if (typeof fetchGlobalUnreadEmails === 'function') fetchGlobalUnreadEmails();
                loadEmails();
            }
...
async function handleAutomatedScan(id) {
    ...
    resultsEl.innerHTML = `<i class="fa-solid fa-magnifying-glass"></i> Processing Code: ` + id;
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) {
            resultsEl.innerHTML = `<i class="fa-solid fa-check"></i> Logged: ` + (data.name || id);
            ...
            resultsEl.innerHTML = `<i class="fa-solid fa-xmark"></i> ` + (data.error || "Failed");
...
function enforceRBAC() {
    ...
    const navQR        = document.querySelector('[data-target="attendance"]');
    const navSchedules = document.querySelector('[data-target="schedules"]');
    ...
    const ROLE_TAB_MAP = {
        'HR':         { dashboard: true,  hr: true,   qr: false,  schedules: false, sme: false, pos: false, transport: false, secretary: false, tech: false },
        'Supervisor': { dashboard: false, hr: false,  qr: true,   schedules: true,  sme: false, pos: false, transport: false, secretary: false, tech: false },
        'CEO':        { dashboard: true,  hr: false,  qr: false,  schedules: false, sme: true,  pos: false, transport: false, secretary: false, tech: false },
        'Cashier':    { dashboard: false, hr: false,  qr: false,  schedules: false, sme: false, pos: true,  transport: true,  secretary: false, tech: false },
        'Transport':  { dashboard: false, hr: false,  qr: false,  schedules: false, sme: false, pos: false, transport: true,  secretary: false, tech: false },
        'Secretary':  { dashboard: false, hr: false,  qr: false,  schedules: false, sme: false, pos: false, transport: false, secretary: true,  tech: false },
    };

    if (isTech || isDemo) {
        // System Technician and Demo mode sees all tabs
        [navDashboard, navHR, navQR, navSchedules, navSME, navPOS, navTransport, navSecretary].forEach(n => { if (n) n.style.display = 'block'; });
        ...
        const hasPerms = Object.values(p).some(v => v !== 0 && v !== null && v !== undefined);
        ...
        if (navDashboard) navDashboard.style.display = perms.dashboard  ? 'block' : 'none';
        if (navHR)        navHR.style.display        = perms.hr         ? 'block' : 'none';
        if (navQR)        navQR.style.display        = perms.qr         ? 'block' : 'none';
        if (navSchedules) navSchedules.style.display = perms.schedules  ? 'block' : 'none';
        if (navSME)       navSME.style.display       = perms.sme        ? 'block' : 'none';
        if (navPOS)       navPOS.style.display       = perms.pos        ? 'block' : 'none';
        if (navTransport) navTransport.style.display = perms.transport  ? 'block' : 'none';
        if (navSecretary) navSecretary.style.display = perms.secretary  ? 'block' : 'none';
        ...
        const p = USER_PERMISSIONS;
        if (navDashboard) navDashboard.style.display = p.can_see_dashboard  ? 'block' : 'none';
        if (navHR)        navHR.style.display        = p.can_see_hr         ? 'block' : 'none';
        if (navQR)        navQR.style.display        = p.can_see_attendance ? 'block' : 'none';
        if (navSchedules) navSchedules.style.display = p.can_see_schedules  ? 'block' : 'none';
        if (navSME)       navSME.style.display       = p.can_see_sme        ? 'block' : 'none';
        if (navPOS)       navPOS.style.display       = p.can_see_pos        ? 'block' : 'none';
        if (navTransport) navTransport.style.display = p.can_see_transport  ? 'block' : 'none';
        if (navSecretary) navSecretary.style.display = p.can_see_secretary  ? 'block' : 'none';
        if (navTechHub)   navTechHub.style.display   = 'none';
        document.querySelectorAll('.tech-only').forEach(el => el.classList.add('hidden'));
        document.querySelectorAll('.admin-only').forEach(el => el.classList.add('hidden'));
    }
    ...
    const firstVisible = [navDashboard, navHR, navQR, navSchedules, navSME, navPOS, navTransport, navSecretary, navTechHub]
    ...
    const btnPosStock = document.getElementById('pos-nav-stock');
    if (btnPosStock) {
        const canSeeStock = ['CEO', 'HR', 'Supervisor', 'Manager', 'Cashier'].includes(USER_ROLE) || isTech;
...
async function fetchAuth(url, options = {}) {
    ...
    if (localStorage.getItem('jomish_demo') === 'true' && options.method && !['GET', 'OPTIONS'].includes(options.method.toUpperCase())) {
        // Silently mock success without throwing errors or toasts
        return new Response(JSON.stringify({ success: true, message: "Demo mode simulated action", id: Date.now() }), { 
            status: 200, 
            headers: {'Content-Type': 'application/json'} 
        });
    }
    ...
    if (!navigator.onLine && window.OfflineDB) {
        if (method === 'GET') {
            ...
            showToast('Offline Mode: Action queued and will sync when online.', 'info');
            return new Response(JSON.stringify({ success: true, message: "Offline queued action", offlineQueued: true, id: Date.now() }), { 
                status: 200, headers: {'Content-Type': 'application/json'} 
            });
        }
    }
            ...
            tr.innerHTML = `
                ...
                <td>
                    <div style="display:flex; gap:5px;">
                        ...
                        ${['CEO', 'HR', 'Supervisor', 'Manager'].includes(USER_ROLE) ? `
...
async function deleteEmployee(id) {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/employees/${id}`, { method: 'DELETE' });
        const data = await res.json();
        if (res.ok) {
            showToast(`Employee terminated. ID revoked & login access permanently deleted.`, 'success');
            loadEmployees();
            loadDashboard();
            document.getElementById('set-password-modal').classList.add('hidden');
        } else {
            showToast(data.error || 'Failed to terminate employee' || 'An error occurred', 'danger');
        }
    } catch (e) {
        console.error(e);
...
async function handleSuspendAccount() {
    ...
    if (!id) return;
    if (!confirm('Suspend this account? The employee will be immediately locked out until restored.')) return;
    
    try {
        const res = await fetchAuth(`${API_URL}/employees/${id}/suspend`, { method: 'PATCH' });
        if (res.ok) {
...
async function handleRestoreAccount() {
    const id = document.getElementById('pass-emp-id').value;
    if (!id) return;
    if (!confirm('Restore this account? The employee will regain login access immediately.')) return;
    
    try {
        const res = await fetchAuth(`${API_URL}/employees/${id}/unsuspend`, { method: 'PATCH' });
        if (res.ok) {
...
async function toggleSick(id, state) {
    if (!confirm(`Mark this staff member as ${state ? 'SICK' : 'HEALTHY'}?`)) return;
    try {
...
async function loadPayrollStatus() {
    ...
    try {
        ...
        if (!data.employees || data.employees.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:30px; color:#94A3B8;">No employees with a configured salary found.</td></tr>';
            return;
        }
        ...
        data.employees.forEach(emp => {
            ...
            const dueDate = new Date(selYear, parseInt(selMonth) - 1, createdDate.getDate());
...
async function processPayroll(empId, empName) {
    ...
    if (!confirm(`Are you sure you want to process payroll for ${empName}?\nAmount: UGX ${amount.toLocaleString()}\nMonth: ${month}\n\nThis will record an expense in the SME portal.`)) return;

    try {
        const res = await fetchAuth(`${API_URL}/payroll`, {
            method: 'POST',
            body: JSON.stringify({ employee_id: empId, month_year: month, amount: amount })
        });
        const data = await res.json();
        ...
        if (res.ok) {
            ...
            loadPayrollStatus();
            printPayrollSlip(empName, "Employee", amount, month, new Date().toISOString());
        } else {
            alert(data.error);
        }
    } catch (e) {
...
function printPayrollList() {
    ...
    const labelMap = { all: 'All Employees', paid: 'Paid Employees', unpaid: 'Unpaid Employees' };
...
async function loadRolesIntoSelect(selectId, currentValue = '') {
    ...
    try {
        ...
        const roles = (data.roles || []).map(r => r.role_name).filter(Boolean).sort();
...
async function handleAddEmployee(e) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            loadEmployees();
            showToast(`✅ ${data.first_name} ${data.last_name} added! Login ID: ${data.employee_code}`, 'success');
            generateIDCard(data);
        } else {
            alert('Add Staff Error: ' + (data.error || 'Check if email is already in use or photo is too large.'));
        }
    } catch (e) {
        console.error(e);
        alert('Network Error: Please ensure you have launched the server using Launch_Jomish_Suite.bat.');
    }
}
...
function generateIDCard(emp) {
    ...
    fetchAuth(`${API_URL}/settings/all`)
        ...
        .then(settings => {
            ...
            const bizName = (settings.business_name || localStorage.getItem('jomish_biz_name') || 'JOMISH SUITE').toUpperCase();
            const contact  = settings.business_contact || localStorage.getItem('jomish_biz_contact') || '';
            const location = settings.business_location || localStorage.getItem('jomish_biz_location') || '';

            // Update localStorage with fresh data
            if (settings.business_name)    localStorage.setItem('jomish_biz_name', settings.business_name);
            if (settings.business_contact) localStorage.setItem('jomish_biz_contact', settings.business_contact);
            if (settings.business_location) localStorage.setItem('jomish_biz_location', settings.business_location);

            const elBizName = document.getElementById('id-biz-name-front');
            if (elBizName) elBizName.innerHTML = `<i class="fa-solid fa-circle-nodes"></i> ${bizName}`;

            const elBizNameBack = document.getElementById('id-biz-name-back');
            if (elBizNameBack) elBizNameBack.innerHTML = `<i class="fa-solid fa-circle-nodes"></i> ${bizName}`;

            // Employee name & role
            const elName = document.getElementById('print-emp-name-front');
            if (elName) elName.textContent = `${emp.first_name || ''} ${emp.last_name || ''}`.trim() || emp.name || 'Unknown';

            const elRole = document.getElementById('print-emp-role-front');
            if (elRole) elRole.textContent = emp.role || 'Staff';

            // Business contact (phone) on front — this is the BUSINESS phone, not employee
            const elContact = document.getElementById('id-contact-info');
            if (elContact) elContact.innerHTML = `<i class="fa-solid fa-phone"></i> ${contact}`;

            const elContactBack = document.getElementById('id-contact-back');
            if (elContactBack) elContactBack.textContent = contact;

            // Employee email (their own email)
            const elEmail = document.getElementById('id-email-info');
            if (elEmail) elEmail.innerHTML = `<i class="fa-solid fa-envelope"></i> ${emp.email || ''}`;

            // Business location
            const elLocation = document.getElementById('id-location-info');
            if (elLocation) elLocation.innerHTML = `<i class="fa-solid fa-location-dot"></i> ${location}`;

            const elLocationBack = document.getElementById('id-location-back');
            if (elLocationBack) elLocationBack.textContent = location;

            // Profile photo
            const elPhotoFront = document.getElementById('id-profile-img-front');
            if (elPhotoFront) {
                elPhotoFront.src = emp.photo_base64 || emp.photo
                    ? (emp.photo_base64 || emp.photo)
                    : 'lib/placeholder.png';
                elPhotoFront.onerror = () => { elPhotoFront.src = 'lib/placeholder.png'; };
            }
        ...
        .catch(() => {
            ...
            const bizName = (localStorage.getItem('jomish_biz_name') || 'JOMISH SUITE').toUpperCase();
            const elBizName = document.getElementById('id-biz-name-front');
            if (elBizName) elBizName.innerHTML = `<i class="fa-solid fa-circle-nodes"></i> ${bizName}`;
            overlay.classList.add('receipt-modal-visible');
            document.body.classList.add('print-mode-id');
...
function clearAllPrintModes() {
    document.body.classList.remove('print-mode-id', 'print-mode-receipt', 'print-mode-label', 'print-mode-timetable', 'print-mode-statement');
}
...
function getCategoryIcon(cat) {
    ...
    if (c.includes('beverage') || c.includes('drink') || c.includes('water')) return '<i class="fa-solid fa-cup-togo"></i>';
    if (c.includes('food') || c.includes('meal')) return '<i class="fa-solid fa-bowl-food"></i>';
    if (c.includes('pastry') || c.includes('bread') || c.includes('cake')) return '<i class="fa-solid fa-bread-slice"></i>';
...
async function loadPOSProducts(searchTerm = '') {
    try {
        ...
        if (filtered.length === 0) {
            grid.innerHTML = '<div style="grid-column: 1/-1; padding: 40px; text-align: center; color: var(--text-muted);"><h3>No Items Found</h3><p>Try searching for something else or check your inventory.</p></div>';
            return;
        }
        filtered.forEach(p => {
...
async function handleBarcodeScan(code) {
    ...
    for (const p of cachedProducts) {
        ...
        if (p.additional_barcodes && p.additional_barcodes.some(b => String(b) === String(code))) {
            bestMatch = p;
        ...
        if (p.barcode_end && !isNaN(code)) {
            ...
            if (!isNaN(startNum) && !isNaN(endNum) && scanNum >= startNum && scanNum <= endNum) {
    ...
    if (bestMatch && !isNaN(code)) {
        ...
        const inCart = posCart.some(item => item.barcodes && item.barcodes.includes(code));
        if (inCart) {
...
function renderCart() {
    ...
    posCart.forEach(item => {
        ...
        const thumbHtml = (item.photo_base64 && item.photo_base64.length > 50)
    ...
    document.getElementById('cart-subtotal').textContent = `UGX ${subtotal.toFixed(0)}`;
    ...
    document.getElementById('btn-checkout').setAttribute('data-total', total.toFixed(0));
}
...
function numpadExact() {
    const total = parseFloat(document.getElementById('btn-checkout').getAttribute('data-total')) || 0;
...
window.handleDeliveryCheckboxChange = function() {
    ...
    if (isDelivery) {
        ...
        if (btnConfirmPay) btnConfirmPay.innerHTML = '<i class="fa-solid fa-truck"></i> CONFIRM & DISPATCH';
        if (btnGetInvoice) btnGetInvoice.style.display = 'none'; // Only allow proper payment confirm for dispatch
    } else {
        deliveryContainer.style.display = 'none';
        if (btnConfirmPay) btnConfirmPay.innerHTML = '<i class="fa-solid fa-check"></i> CONFIRM PAYMENT';
...
async function confirmPayment() {
    ...
    if (paymentMethod === 'CREDIT') {
        ...
        if (paid < Math.round(total)) {
            alert('Amount paid is less than total! Customer still owes UGX ' + (Math.round(total) - paid).toLocaleString());
            return;
        }
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            printReceipt(posCart, total, result.transaction_id, paymentMethod, paid, buyerName, null);
...
async function getInvoice() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            printReceipt(posCart, total, result.transaction_id, 'INVOICE', 0, buyerName, null, true);
...
function printReceipt(
    ...
    const bizLoc  = localStorage.getItem('jomish_biz_location') || 'Kampala, Uganda';
    ...
    const tax    = total - sub;
    const change = (paymentMethod !== 'CREDIT' && paymentMethod !== 'INVOICE') ? Math.max(0, paid - total) : 0;
    const bal    = (paymentMethod === 'CREDIT' || paymentMethod === 'INVOICE')  ? Math.max(0, total - paid) : 0;
    const orderNo = String(orderId || Math.floor(Math.random() * 9999)).padStart(4, '0');
    const dateStr = new Date().toLocaleString('en-UG', { timeZone: 'Africa/Kampala' });

    // ── Store receipt data for the print window ────────────
    _currentReceiptData = {
        isInvoice, bizName, bizLoc, bizTel, taxRate,
        orderId: orderNo, dateStr,
    ...
    set('r-tax',      taxRate > 0 ? `UGX ${Math.round(tax).toLocaleString()} (${taxRate}%)` : 'UGX 0');
    ...
    if (deliveryInfo) {
        if (typeof deliveryInfo === 'object') {
            set('r-delivery-client', `${deliveryInfo.clientName || ''} ${deliveryInfo.clientPhone ? '(' + deliveryInfo.clientPhone + ')' : ''}`.trim());
...
function _buildReceiptHTML(d) {
    ...
    let deliveryBlock = '';
    if (d.deliveryInfo) {
        const clientStr = typeof d.deliveryInfo === 'object'
            ? `${d.deliveryInfo.clientName || ''} ${d.deliveryInfo.clientPhone ? '(' + d.deliveryInfo.clientPhone + ')' : ''}`.trim()
            : String(d.deliveryInfo);
        const locStr = typeof d.deliveryInfo === 'object' ? (d.deliveryInfo.clientLocation || '') : '';
        deliveryBlock = `
    ...
    const logoSrc = localStorage.getItem('jomish_logo_base64') || 'assets/default-logo.png';
...
async function updateRole(roleName, module, isChecked) {
    const tbody = document.querySelector('#roles-table tbody');
    let row;
    for (let tr of tbody.children) { if (tr.children[0].innerText === roleName) { row = tr; break; } }
    if (!row) return;
    ...
    const payload = {
        ...
        can_see_system_users: row.children[9].querySelector('input').checked ? 1 : 0,
...
window.promptSetRolePassword = function(roleName) {
    ...
    document.getElementById('confirm-pwd-input').placeholder = 'Type new portal password…';
...
async function submitConfirmPassword() {
    ...
    if (!roleName) { document.getElementById('confirm-password-modal').classList.add('hidden'); return; }

    try {
        const res = await fetchAuth(`${API_URL}/roles/${encodeURIComponent(roleName)}/password`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ password: pwd })
        });
        const data = await res.json();
        if (!res.ok) { errEl.textContent = data.error || 'Failed to set password.'; errEl.style.display = 'block'; return; }
...
async function loadBrandLogo() {
    try {
        ...
        if (data._is_global_tech) {
            ...
            if (sideBrand) { sideBrand.innerText = 'Jomish Suite'; sideBrand.style.display = ''; }
        ...
        if (data.business_name) {
            ...
            const sideBrandLegacy = document.querySelector('.brand h2:not(#sidebar-brand-name)');
            ...
            if (idBrand) idBrand.innerText = bizName.toUpperCase();
        ...
        if (data.business_modules) {
            try {
                const modules = typeof data.business_modules === 'string'
                ...
                ['dashboard', 'hr', 'attendance', 'sme', 'pos', 'secretary', 'transport', 'schedules', 'hardware'].forEach(m => {
                ...
                if (cbSysUser && modules.system_users !== undefined) cbSysUser.checked = !!modules.system_users;
            } catch (e) {
        ...
        if (data.global_shifts) {
            try {
                const s = typeof data.global_shifts === 'string' ? JSON.parse(data.global_shifts) : data.global_shifts;
                if (Array.isArray(s) && s.length > 0) {
                    GLOBAL_SHIFTS = s;
                }
            } catch(e) { console.error('Failed to parse global_shifts', e); }
        }
        if (data.global_operational_days) {
            try {
                const d = typeof data.global_operational_days === 'string' ? JSON.parse(data.global_operational_days) : data.global_operational_days;
                if (Array.isArray(d)) {
                    GLOBAL_OPERATIONAL_DAYS = d;
                }
            } catch(e) { console.error('Failed to parse global_operational_days', e); }
        }
...
async function handleLogoUpload() {
    ...
    reader.onload = async (e) => {
        const res = await fetchAuth(`${API_URL}/settings/logo`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ logo_base64: e.target.result }) });
        if (res.ok) {
...
async function handleSaveModulesUpdate() {
    ...
    const modules = {
        ...
        transport: document.getElementById('mod-transport') ? document.getElementById('mod-transport').checked : true,
        schedules: document.getElementById('mod-schedules') ? document.getElementById('mod-schedules').checked : true,
        hardware: document.getElementById('mod-hardware') ? document.getElementById('mod-hardware').checked : true,
        system_users: document.getElementById('mod-system-users') ? document.getElementById('mod-system-users').checked : true
    };
    ...
    try {
        ...
        if (btn) { btn.disabled = false; btn.textContent = 'Save Custom Active Tabs'; }
    }
}
...
window.addTechShift = function() {
    GLOBAL_SHIFTS.push({ id: 'shift_'+Date.now(), label: 'New Shift', icon: 'fa-solid fa-clock', color: '#6366F1', start_time: '08:00', end_time: '17:00' });
    renderTechShifts();
};
...
window.saveTechShifts = async function() {
    for (let i=0; i<GLOBAL_SHIFTS.length; i++) {
        const label = document.getElementById('tech-shift-label-'+i).value || 'Shift '+i;
        GLOBAL_SHIFTS[i].label = label;
        GLOBAL_SHIFTS[i].id = label.toLowerCase().replace(/[^a-z0-9]/g, '');
        GLOBAL_SHIFTS[i].start_time = document.getElementById('tech-shift-start-'+i).value || '00:00';
        GLOBAL_SHIFTS[i].end_time = document.getElementById('tech-shift-end-'+i).value || '00:00';
    }
    
    GLOBAL_OPERATIONAL_DAYS = Array.from(document.querySelectorAll('#tech-shift-days input:checked')).map(i => i.value);
    ...
    try {
        ...
        const res2 = await fetchAuth(`${API_URL}/settings`, {
            method: 'POST',
            body: JSON.stringify({ key: 'global_operational_days', data: GLOBAL_OPERATIONAL_DAYS })
        });
        if (res1.ok && res2.ok) {
    ...
    } catch(e) { console.error(e); alert('Error saving shifts'); }
};



async function loadAttendance() {
    console.log("Attendance: Starting data load...");
    ...
    try {
        console.log("Attendance: Fetching present staff...");
        const presRes = await fetchAuth(`${API_URL}/attendance/present`);
        const presData = await presRes.json();
        console.log("Attendance: Present staff received:", presData);
        ...
        if (pTbody) {
            ...
            if (present.length === 0) {
                pTbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:20px; color:var(--text-muted);">No staff currently in building.</td></tr>';
            } else {
                present.forEach(e => {
                    const tr = document.createElement('tr');
            ...
            console.error("Attendance: Table #present-staff-table tbody not found in DOM!");
        }
    } catch (e) { console.error("Presence Monitoring Error:", e); }
}
...
async function loadTransactions(searchTerm = '') {
    try {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(tx => 
                ...
                String(tx.id).padStart(4, '0').includes(term) ||
    ...
    } catch (e) { console.error('TX Network Error:', e); }
}
...
async function viewReceiptDetails(txId) {
    try {
        ...
        document.getElementById('rd-tx-id').textContent = `RCPT-${String(txId).padStart(4, '0')}`;
        document.getElementById('rd-cashier').textContent = order.cashier_name || 'System';
        document.getElementById('rd-date').textContent = formatDisplayDate(order.order_date, true);
        
        const tbody = document.getElementById('rd-items-body');
        tbody.innerHTML = '';
        
        if (!order.items || order.items.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:var(--text-muted); padding:20px;">Itemized data not available for this legacy receipt.</td></tr>';
        } else {
        ...
        document.getElementById('rd-grand-total').textContent = `UGX ${order.total_amount.toLocaleString()}`;
...
async function populateCashierSelect() {
    ...
    } catch (e) { console.error('Error populating cashiers:', e); }
}
...
async function loadCashierReport() {
    ...
    try {
        ...
        if (!data.reports || data.reports.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--text-muted);">No records found in this period.</td></tr>';
            return;
        }
...
async function deleteTransaction(id) {
    ...
    if (!confirm(`<i class="fa-solid fa-triangle-exclamation"></i> WARNING: Are you absolutely sure you want to permanently delete transaction RCPT-${receiptNo}? This action is irreversible and will affect the financial portal history.`)) {
        return;
    }
    
    try {
        const res = await fetchAuth(`${API_URL}/transactions/${id}`, { method: 'DELETE' });
        if (res.ok) {
            showToast(`Transaction RCPT-${receiptNo} deleted successfully.`, 'success');
            loadTransactions();
            loadDashboard();
        } else {
            const data = await res.json();
...
async function resetDatabase() {
    ...
    promptOverlay.style = "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); z-index:9999; display:flex; justify-content:center; align-items:center;";
    ...
    document.getElementById('reset-confirm-btn').onclick = async () => {
        ...
        if (pass2 !== "JomishRecovery99!!" && pass2 !== "Jomish9!!") {
            document.body.removeChild(promptOverlay);
            alert("Verification failed. Incorrect technician password. Reset aborted.");
            return;
        }
        document.body.removeChild(promptOverlay);
        ...
        try {
            const res = await fetchAuth(`${API_URL}/system/reset`, { method: 'POST' });
            if (res.ok) {
                alert("Database successfully wiped. Seed accounts created:\n- CEO: ceo / ceo123\n- HR: admin / admin123\n\nYou will now be redirected to log in with these new credentials.");
...
async function handleRestore(input) {
    ...
    if (!confirm(`<i class="fa-solid fa-bell"></i> DANGER: You are about to completely OVERWRITE the current database with "${file.name}". All existing data will be permanently lost and replaced by this backup. Are you absolutely sure you want to proceed?`)) {
        input.value = '';
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            alert("Database restored successfully! The system will now restart to apply changes. Please wait a moment and then log back in.");
            localStorage.clear();
            setTimeout(() => {
                window.location.href = 'login.html';
            }, 2000);
...
async function loadPettyCash() {
    const token = localStorage.getItem('jomish_token');
    try {
        const shiftRes = await fetchAuth(`${API_URL}/shifts/status`);
        if (!shiftRes || !shiftRes.id) {
            document.getElementById('petty-cash-body').innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--text-muted);">No active shift. Petty cash disabled.</td></tr>`;
            return;
        }
        _currentPettyCashShiftId = shiftRes.id;
        
        const res = await fetch(`${API_URL}/petty-cash?shift_id=${_currentPettyCashShiftId}`, { headers: { 'Authorization': `Bearer ${token}` } });
        ...
        if (!entries.length) {
            ...
            tbody.innerHTML = entries.map(e => {
                ...
                const date = new Date(e.created_at.replace(' ', 'T')).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
                return `<tr>
                    <td>${e.purpose}</td>
        ...
        document.getElementById('petty-cash-total').textContent = `Total: UGX ${total.toLocaleString()}`;
        
        // Show/hide admin-only actions (like delete)
        document.querySelectorAll('.admin-only').forEach(el => {
            if (USER_ROLE === 'CEO' || USER_ROLE === 'HR') el.classList.remove('hidden');
        });
    } catch (e) {
...
async function handlePettyCashSubmit(e) {
    e.preventDefault();
    if (!_currentPettyCashShiftId) return alert('No active shift. Cannot log petty cash.');
    const purpose = document.getElementById('pc-purpose').value.trim();
    const amount = document.getElementById('pc-amount').value;
    const token = localStorage.getItem('jomish_token');
    const body = { shift_id: _currentPettyCashShiftId, purpose, amount, recorded_by: window.USER_NAME || 'Unknown' };
...
async function deletePettyCash(id) {
    ...
    const token = localStorage.getItem('jomish_token');
    await fetch(`${API_URL}/petty-cash/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
    await loadPettyCash();
}
...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('form-petty-cash')?.addEventListener('submit', handlePettyCashSubmit);
    // Bind it to the schedules navigation button to reload when they click the tab
    document.querySelectorAll('.nav-btn[data-target="schedules"]').forEach(btn => {
        btn.addEventListener('click', () => {
            if (['Supervisor', 'HR', 'CEO'].includes(window.USER_ROLE)) {
                loadPettyCash();
            }
        });
    });
...
async function loadDeliveries() {
    const token = localStorage.getItem('jomish_token');
    try {
        const res = await fetch(`${API_URL}/deliveries`, { headers: { 'Authorization': `Bearer ${token}` } });
        if (!res.ok) throw new Error(await res.text());
...
const STATUS_COLORS = { 'Pending': '#F59E0B', 'In Transit': '#3B82F6', 'Delivered': '#10B981', 'Failed': '#EF4444' };

function renderDeliveries(rows) {
    const tbody = document.getElementById('deliveries-table-body');
    if (!tbody) return;
    const filter = (document.getElementById('trx-filter-status')?.value || '').toLowerCase();
    ...
    const filtered = rows.filter(r => {
        ...
        const matchSearch = !search || r.client_name?.toLowerCase().includes(search) || r.client_phone?.toLowerCase().includes(search);
    ...
    tbody.innerHTML = filtered.map(r => {
        ...
        const canClaim = USER_ROLE === 'Driver' && r.status === 'Pending' && !r.driver_id;
        const myJobBadge = (USER_ROLE === 'Driver' && r.driver_name === USER_NAME) ? `<span style="background:var(--success);color:white;padding:2px 6px;border-radius:4px;font-size:0.7rem;margin-left:5px;">My Job &#10003;</span>` : '';
        const claimBtn = canClaim ? `<button onclick="claimDelivery(${r.id})" class="sm-btn success" style="margin-right:5px;">Take Job</button>` : '';
...
async function deleteDelivery(id) {
    ...
    const token = localStorage.getItem('jomish_token');
    await fetch(`${API_URL}/deliveries/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
    await loadDeliveries();
}
...
async function claimDelivery(id) {
    const token = localStorage.getItem('jomish_token');
    try {
        const res = await fetch(`${API_URL}/deliveries/${id}/claim`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` } });
        if (res.ok) {
...
async function handleNewDeliverySubmit(e) {
    ...
    const body = {
        ...
        client_location: document.getElementById('del-client-location').value.trim(),
...
document.querySelectorAll('.nav-btn[data-target="transport-hub"]').forEach(btn => {
...
document.getElementById('btn-close-delivery-modal')?.addEventListener('click', () => {
    const m = document.getElementById('delivery-modal');
    if (m) { m.classList.add('hidden'); m.style.cssText = ''; }
});
document.getElementById('form-new-delivery')?.addEventListener('submit', handleNewDeliverySubmit);

// Wire filter/search
document.getElementById('trx-filter-status')?.addEventListener('change', () => renderDeliveries(_deliveries));
document.getElementById('trx-search')?.addEventListener('input', () => renderDeliveries(_deliveries));
...
async function openEmployeeNotes(id, name) {
    ...
    document.getElementById('note-emp-name').innerText = `Performance Notes: ${name}`;
...
async function loadEmployeeNotes(id) {
    try {
        ...
        if (!data.notes || data.notes.length === 0) {
            list.innerHTML = '<p style="text-align:center; color:#94A3B8; padding:20px;">No performance notes found for this employee.</p>';
            return;
        }
        ...
        data.notes.forEach(n => {
            ...
            const badgeColor = n.note_type === 'POSITIVE' ? '#10B981' : (n.note_type === 'WARNING' ? '#EF4444' : '#6366F1');
...
async function handleAddNote(e) {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/notes`, {
            method: 'POST',
            body: JSON.stringify({ employee_id: id, note_text: text, note_type: type })
        });
        if (res.ok) {
...
async function handleScan() {
    ...
    if (!identifier) {
        document.getElementById('qr-reader-results').innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> Please type an ID or use the Camera.';
        return;
    }
    const res = await fetchAuth(`${API_URL}/attendance/scan`, { 
        method: 'POST', 
    ...
    const data = await res.json();
    if (res.ok) {
        document.getElementById('qr-reader-results').innerHTML = (data.already_in ? '<i class="fa-solid fa-circle-info"></i> ' : "<i class='fa-solid fa-check'></i> ") + data.message;
...
async function loadSoldBarcodes() {
    try {
        ...
        if (data.sold_barcodes) {
            ...
            console.log(`[POS] Loaded ${soldBarcodes.size} unique sold barcodes.`);
        }
    } catch (e) { console.error('Load sold barcodes error:', e); }
}
...
async function loadInventory() {
    try {
        ...
        if (lowStockCountEl) {
            ...
            lowStockCountEl.parentElement.parentElement.style.background = lowStock.length > 0 ? '#FEF3C7' : 'var(--surface)';
        }
        ...
        data.products.forEach(p => {
            ...
            const imageHtml = (p.photo_base64 && p.photo_base64.length > 50)
...
async function handleAddProduct(e) {
    ...
    const payload = {
        ...
        buying_price: parseFloat(document.getElementById('add-prod-buying-price').value) || 0,
        photo_base64: photoBase64
    };
    ...
    try {
        ...
        if (res.ok) {
            ...
            const targetQty = parseInt(document.getElementById('add-prod-qty').value) || 1;
            ...
            const data = await res.json();
            alert('Error saving product: ' + (data.error || 'Check server connection'));
        }
    } catch (err) {
        console.error(err);
...
window.closeScanSession = function() {
    ...
    loadInventory();
    loadPOSProducts();
    setTimeout(() => { window._skipProductReload = false; }, 2000);
}
...
window.submitScanInput = async function() {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/products/${_currentScanProductId}/scan-assign`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.generateBulkBarcodes = async function() {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/products/${_currentScanProductId}/generate-labels-bulk`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.ignoreRemainingBarcodes = async function() {
    ...
    try {
        ...
        _scanCount += remaining;
        document.getElementById('scan-count').textContent = _scanCount;
        document.getElementById('scan-last-status').textContent = `[OK] Stock added.`;
...
async function deleteProduct(id) {
    ...
    const res = await fetchAuth(`${API_URL}/products/${id}`, { method: 'DELETE' });
    if (res.ok) {
...
// ==== EDIT PRODUCT ====
function openEditProduct(id, name, category, price, barcode, barcodeEnd, buyingPrice = 0) {
...
async function handleEditProduct(e) {
    ...
    const newBarcode = document.getElementById('edit-prod-barcode').value.trim().toLowerCase();
    ...
    if (duplicate) {
        alert(`Duplicate Barcode: This barcode is already assigned to '${duplicate.name}'.`);
        return;
    }
    ...
    const payload = {
        ...
        buying_price: parseFloat(document.getElementById('edit-prod-buying-price').value) || 0,
...
async function loadSystemUsers(searchTerm = '') {
    try {
        ...
        if (filtered.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--text-muted);">No system users found.</td></tr>';
            return;
        }
...
async function handleUpdateAccess(e) {
    ...
    if (empId) {
        ...
        if (res.ok) {
            ...
            const data = await res.json();
            alert('Error: ' + (data.error || 'Server rejected the update. Please RESTART the server.'));
        }
    } else {
        ...
        if (res.ok) {
            ...
            const data = await res.json();
            alert('Error: ' + (data.error || 'Could not create user. Check the server.'));
        }
    }
}
...
function autoRedirect() {
    if (USER_ROLE === 'Cashier' || USER_ROLE === 'Finance Manager') {
        ...
        if (posBtn && posBtn.style.display !== 'none') {
            ...
            const fallback = document.querySelector('.sidebar nav .nav-btn:not([style*="display: none"])');
            if (fallback) fallback.click();
        }
    } else if (USER_ROLE === 'Security') {
        const attBtn = document.querySelector('[data-target="attendance"]');
...
async function loadSystemStatus() {
    try {
        ...
        if (dbStatus) {
            dbStatus.innerText = data.dbType.toUpperCase() + " Engine";
...
window.printTimetable = async function() {
    ...
    const companyName = document.getElementById('brand-name')?.innerText || 'Jomish Business Suite';
    ...
    const from = parts[0];
    const to = parts[1];
    if (!from || !to) { alert('Timetable not loaded yet. Please wait.'); return; }

    // Fetch live assignments
    let assignments = [];
    try {
        const res = await fetchAuth(`${API_URL}/shift-assignments?from=${from}&to=${to}`);
        const data = await res.json();
        assignments = data.assignments || [];
    } catch(e) { alert('Could not load timetable data.'); return; }
    ...
    const dateLabels  = days.map(d => d.toLocaleDateString('en-UG', { day:'2-digit', month:'short' }));

    // Build per-employee map: id → {name, role, days: {dateStr → {slot, time}}}
    const empMap = {};
    assignments.forEach(a => {
        const key = a.employee_id;
        if (!empMap[key]) empMap[key] = { name: `${a.first_name} ${a.last_name}`, role: a.role, dayData: {} };
        empMap[key].dayData[a.shift_date] = { slot: a.slot || '', start: a.start_time || '', end: a.end_time || '' };
    });

    let employees = Object.values(empMap).sort((a,b) => a.name.localeCompare(b.name));

    // Ensure at least 15 rows (blank rows like the template)
    while (employees.length < 15) employees.push({ name: '', role: '', dayData: {} });
    ...
    const rowsHtml = employees.map((emp, idx) => {
        ...
        const dayCells = days.map(d => {
            const ds = d.toISOString().split('T')[0];
            const info = emp.dayData[ds];
            const cellText = info ? `${info.slot}${info.start ? '<br><small>' + info.start + '-' + info.end + '</small>' : ''}` : '';
    ...
    if (!printWindow) { alert('Please allow popups to print the timetable.'); return; }
...
<body>
    ...
    <div class="notes-section">
        ...
        ${Array.from({length:4}).map(() => `<div class="notes-row"></div>`).join('')}
    </div>
...
async function printProductLabelsRange(productName, startNum, endNum, price) {
    ...
    if (count > 200 && !confirm(`This will generate ${count} labels. Continue?`)) return;
    ...
    for (let i = 0; i < count; i++) {
        ...
        nameEl.style = "font-size: 5pt; font-family: helvetica, sans-serif; font-weight: bold; text-align: center; line-height: 1.1; width: 100%; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;";
        nameEl.textContent = productName.toUpperCase();
        
        const priceEl = document.createElement('div');
        priceEl.style = "font-size: 4.5pt; font-family: helvetica, sans-serif; text-align: center; margin-bottom: 2px;";
        if (price) {
        ...
        try {
            JsBarcode(svg, currentCode, { format: 'CODE128', width: 1.5, height: 50, displayValue: false, margin: 10 });
        } catch (e) {
            JsBarcode(svg, currentCode, { format: 'codabar', width: 1.5, height: 50, displayValue: false, margin: 10 });
        }

        const codeEl = document.createElement('div');
        codeEl.style = "font-size: 6pt; font-family: 'Courier New', monospace; font-weight: bold; text-align: center; margin-top: 1px;";
        codeEl.textContent = currentCode;

        const footerEl = document.createElement('div');
        footerEl.style = "font-size: 2.5pt; font-family: helvetica, sans-serif; text-align: center; color: #666; margin-top: auto;";
    ...
    document.getElementById('label-overlay').classList.add('receipt-modal-visible');
...
function initBarcodeAutoCalc() {
    ...
    if (barcodeNote) barcodeNote.closest('.form-group')?.setAttribute('style', 'display:none');
}
...
function switchSecretaryView(view) {
    document.getElementById('sec-calendar-view').style.display = view === 'calendar' ? 'block' : 'none';
    document.getElementById('sec-inbox-view').style.display = view === 'inbox' ? 'block' : 'none';
    ...
    const waView = document.getElementById('sec-whatsapp-view');
    if (waView) waView.style.display = view === 'whatsapp' ? 'block' : 'none';
    ...
    const waBtn = document.getElementById('btn-sec-whatsapp');
    if (waBtn) waBtn.classList.toggle('active', view === 'whatsapp');
    const meetBtn = document.getElementById('btn-sec-meet');
    if (meetBtn) meetBtn.classList.toggle('active', view === 'meet');

    if (view === 'calendar') renderCalendar();
    if (view === 'inbox') { loadEmails(); loadEmailConfig(); }
    if (view === 'messages') { loadStaffList(); loadMessages(); }
    if (view === 'directory') renderStaffDirectory();
    if (view === 'meet') loadMeetingOptions();
    if (view === 'whatsapp') loadWhatsAppStatus();
...
async function loadMeetingOptions() {
    try {
        ...
        const res = await fetchAuth(`${API_URL}/calendar/events?month=${m}&year=${y}`);
        const data = await res.json();
        ...
        const todaysEvents = (data.events || []).filter(e => e.event_date === todayStr);
        if (todaysEvents.length === 0) {
            select.innerHTML = '<option value="">No meetings scheduled for today</option>';
        } else {
...
async function saveMeetingMinutes() {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/calendar/events/${eventId}/minutes`, {
            method: 'POST',
...
async function renderCalendar() {
    ...
    try {
        const m = calMonth + 1;
        const res = await fetchAuth(`${API_URL}/calendar/events?month=${m}&year=${calYear}`);
        const data = await res.json();
        calEvents = data.events || [];
    } catch(e) { console.error('Calendar fetch error:', e); calEvents = []; }
    ...
    for (let i = 0; i < totalCells; i++) {
        ...
        if (dayEvents.length > 3) {
            ...
            more.style.cssText = 'font-size:0.6rem; color:var(--text-muted); padding-left:5px;';
...
function showDayEvents(dateStr) {
    ...
    dayEvents.forEach(evt => {
        ...
        const timeStr = evt.start_time ? `${evt.start_time}${evt.end_time ? ' - ' + evt.end_time : ''}` : 'All Day';
        const typeColors = { Meeting:'#4F46E5', Appointment:'#10B981', Deadline:'#EF4444', Reminder:'#F59E0B', Holiday:'#EC4899', Other:'#8B5CF6' };
        card.innerHTML = `
            <div style="flex:1;">
...
async function handleCalEventSubmit(e) {
    ...
    if (!title) { alert('ERROR: Title is empty. Please enter an event title.'); return; }
    if (!event_date) { alert('ERROR: Date is empty. Please pick a date.'); return; }

    const payload = { title, event_type, event_date, start_time, end_time, description, color };

    // STEP 3: Check token
    const token = localStorage.getItem('jomish_token');
    if (!token) { alert('ERROR: No login token found. You are not logged in.'); return; }
    ...
    try {
        ...
        if (res.ok) {
            ...
            try { const d = await res.json(); errMsg = d.error || errMsg; } catch(je) {}
...
async function deleteCalEvent(id) {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/calendar/events/${id}`, { method: 'DELETE' });
...
function openMinutesModal(eventId) {
    ...
    document.getElementById('minutes-modal-title').textContent = `Minutes: ${evt.title}`;
    
    const timeStr = evt.start_time ? `${evt.start_time}${evt.end_time ? ' - ' + evt.end_time : ''}` : 'All Day';
    document.getElementById('minutes-modal-subtitle').innerHTML = `<i class="fa-solid fa-calendar"></i> ${evt.event_date} | <i class="fa-regular fa-clock"></i> ${timeStr}`;
...
document.getElementById('btn-save-minutes')?.addEventListener('click', async () => {
...
document.getElementById('btn-download-minutes')?.addEventListener('click', () => {
    const id = document.getElementById('minutes-evt-id').value;
    const evt = calEvents.find(e => e.id == id);
    if (!evt) return;
    ...
    try {
        ...
        doc.save(`Meeting_Minutes_${evt.event_date}_${evt.title.replace(/[^a-z0-9]/gi, '_')}.pdf`);
    } catch(e) {
...
async function loadEmails() {
    try {
        ...
        if (typeof updateSecretaryGlobalBadge === 'function') updateSecretaryGlobalBadge();

        const tbody = document.getElementById('email-inbox-tbody');
        if (emails.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:40px; color:#94A3B8;">No emails yet. Click "Fetch New Emails" to pull from Gmail.</td></tr>';
            return;
        }
    ...
    } catch(e) { console.error('Load emails error:', e); }
}
...
async function openEmailPreview(em) {
    ...
    document.getElementById('capsule-from').textContent = em.from_name || em.from_address;
    document.getElementById('capsule-date').textContent = formatDisplayDate(em.received_at);
    document.getElementById('capsule-body').textContent = em.body_preview || '(No content)';
...
function bridgeStaffEmail(email, name, content) {
    ...
    document.getElementById('reply-subject').value = `Follow-up: Internal Message for ${name}`;
    document.getElementById('reply-body').value = `Hi ${name.split(' ')[0]},\n\nI'm following up on our internal message:\n"${content}"\n\n---\nBest Regards,\n${localStorage.getItem('jomish_name')}`;
...
async function fetchNewEmails(e) {
    const btn = e && e.target ? e.target : document.querySelector('[onclick*="fetchNewEmails"]');
    if (btn) { btn.disabled = true; btn.textContent = 'Syncing Gmail...'; }
    try {
        ...
        if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-inbox"></i> Sync Inbox'; }
    }
}
...
function openNewEmailCapsule() {
    ...
    // Set Header
    document.getElementById('capsule-reply-view').querySelector('h2').innerHTML = '<i class="fa-solid fa-envelope"></i> Compose New Email';
...
function openBlastEmailCapsule() {
    // Set Blast Mode
    document.getElementById('reply-to').value = '<i class="fa-solid fa-bullhorn"></i> ALL ACTIVE STAFF (BCC Blast)';
    ...
    // Set Header
    document.getElementById('capsule-reply-view').querySelector('h2').innerHTML = '<i class="fa-solid fa-bullhorn"></i> Mass Staff Email Blast';
...
async function handleEmailReplySubmit(e) {
    ...
    const endpoint = isBlast ? `${API_URL}/emails/blast` : `${API_URL}/emails/send`;
    ...
    try {
        ...
        if (res.ok) {
            alert(isBlast ? 'Staff Blast sent successfully!' : 'Email sent successfully!');
            closeEmailCapsule();
            e.target.reset();
        } else {
            const data = await res.json();
...
async function loadMessages() {
    try {
        ...
        if (msgs.length === 0) {
            container.innerHTML = '<p style="color:var(--text-muted); text-align:center; padding:30px;">No messages yet. Send your first message to a staff member!</p>';
            return;
        }
        ...
        msgs.forEach(m => {
            ...
            const otherName = isSent ? `${m.to_first || ''} ${m.to_last || ''}` : `${m.from_first || ''} ${m.from_last || ''}`;
            ...
            if (!m.is_read && !isSent) {
                ...
                card.onclick = async () => {
                    await fetchAuth(`${API_URL}/messages/${m.id}/read`, { method: 'PATCH' });
                    loadMessages();
                };
            }
            container.appendChild(card);
...
function searchStaffDirectory() {
    ...
    const filtered = (window._staffDirectory || []).filter(emp => {
        ...
        const searchStr = `${emp.first_name} ${emp.last_name} ${emp.email} ${emp.role} ${emp.department}`.toLowerCase();
        return searchStr.includes(query);
    });

    if (filtered.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:30px; color:var(--text-muted);">No staff found matching your search.</td></tr>';
        return;
    }
...
async function handleSendMessage(e) {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/messages`, { method: 'POST', body: JSON.stringify(payload) });
        if (res.ok) {
            alert('Message sent!');
            e.target.reset();
            loadMessages();
...
async function loadDeskMessages() {
    ...
    if (USER_ROLE === 'Receptionist') { panel.style.display = 'none'; return; }
    ...
    try {
        ...
        if (received.length === 0) { panel.style.display = 'none'; return; }
    ...
    } catch(e) { console.error('Desk messages error:', e); }
}
...
async function handleReplyToDesk(e) {
    ...
    if (!_receptionistId) {
        ...
        try {
            const res = await fetchAuth(`${API_URL}/employees`);
            const data = await res.json();
            const rec = (data.employees || []).find(emp => emp.role === 'Receptionist' && emp.is_active);
            if (rec) _receptionistId = rec.id;
            else { alert('No active Receptionist found in the system.'); return; }
        } catch(e) { alert('Could not find Receptionist.'); return; }
    }
    const payload = {
...
function dismissDataLoss() {
    ...
    fetchAuth(`${API_URL}/system-meta/data_loss_warning`, { method: 'DELETE' }).catch(() => {});
}

// ===== SECRETARY GLOBAL BADGE LOGIC =====
window.unreadEmails = 0;
window.unreadWhatsApp = 0;

function updateSecretaryGlobalBadge() {
    const badge = document.getElementById('sec-global-badge');
    if (!badge) return;
    const total = (window.unreadEmails || 0) + (window.unreadWhatsApp || 0);
    if (total > 0) {
...
function initGlobalScanner() {
    console.log("[SCANNER] Global Hardware Listener Active");
...
async function handleGlobalBarcode(code) {
    console.log("[SCANNER] Code Received:", code);
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (result.type === 'EMPLOYEE') {
                ...
                showToast(result.message, result.action === 'IN' ? 'success' : 'warning');
                if (USER_PERMISSIONS.can_see_attendance) loadAttendance();
                loadDashboard();
            } else if (result.type === 'PRODUCT') {
                const product = result.data;
                showToast(`${product.name} — UGX ${Number(product.price).toLocaleString()} (Stock: ${product.stock})`, 'info');
            }
        } else {
...
async function loadCredits() {
    try {
        ...
        if (res.ok) {
            ...
            const sortedGroups = Object.values(groups).sort((a, b) => b.latest_date - a.latest_date);
            window.creditGroups = groups; // Store globally for printStatement

            if (sortedGroups.length === 0) {
                container.innerHTML = '<div style="text-align:center; padding:30px; color:#94A3B8;">No credit records found.</div>';
                return;
            }
            ...
            sortedGroups.forEach(g => {
                ...
                const balanceText = g.total_balance > 0 ? `UGX ${g.total_balance.toLocaleString()}` : 'CLEARED';
            ...
            if (errContainer) errContainer.innerHTML = '<div style="text-align:center; padding:30px; color:#EF4444;">Failed to load. Please make sure the server has been restarted.</div>';
        }
    } catch (err) {
        console.error("Failed to load credits:", err);
        const errContainer = document.getElementById('credits-list-container');
        if (errContainer) errContainer.innerHTML = '<div style="text-align:center; padding:30px; color:#EF4444;">Connection error. Is the server running?</div>';
    }
}
...
function openCreditPaymentModal(buyerName, totalBalance) {
    ...
    setTimeout(() => document.getElementById('credit-payment-amount').focus(), 100);
}
...
async function submitCreditPayment(e) {
    ...
    try {
        const res = await fetchAuth(`${API_URL}/credits/pay`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ buyer_name: buyerName, amount_paid: amount, note })
        });
        const data = await res.json();
        if (res.ok) {
...
function printStatement(buyerName) {
    ...
    const bizLocation = localStorage.getItem('jomish_biz_location') || 'Kampala, Uganda';
...
(function() {
    ...
    function fmtShortDate(d) {
        return d.toLocaleDateString('en-UG', { weekday:'short', month:'short', day:'numeric' });
    }
    ...
    function injectTimetableButtons() {
        ...
        if (CAN_AUTOGEN()) {
            ...
            autoBtn.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-Schedule Week';
    ...
    async function loadAllEmployees() {
        ...
        } catch(e) { console.error('[Timetable] loadAllEmployees error:', e); }
    }
    ...
    async function renderTimetable() {
        ...
        if (headerRow) {
            ...
            days.forEach(d => {
                const todayStyle = isToday(d) ? 'background:rgba(99,102,241,0.08); color:var(--primary);' : '';
                headerRow.innerHTML += `
        ...
        GLOBAL_SHIFTS.forEach(shift => {
            ...
            days.forEach(d => {
                ...
                const key  = `${slot}|${ds}`;
                const list = idx[key] || [];
                const todayStyle = isToday(d) ? 'background:rgba(99,102,241,0.04);' : '';
        ...
        if (el('count-active-shifts')) el('count-active-shifts').textContent = `${assignments.length} Shifts`;
        if (el('count-deployed'))      el('count-deployed').textContent      = uniqueDeployed;
        if (el('count-gaps'))          el('count-gaps').textContent          = totalSlots - filledSlots;
        if (el('count-coverage')) {
            const pct = totalSlots > 0 ? Math.round(filledSlots / totalSlots * 100) : 0;
            el('count-coverage').textContent = `${pct}%`;
            el('count-coverage').style.color = pct >= 80 ? '#10B981' : pct >= 50 ? '#F59E0B' : '#EF4444';
        }
    ...
    function renderFairnessBars(assignments) {
        ...
        if (Object.keys(countMap).length === 0) {
            container.innerHTML = '<p style="color:var(--text-muted); font-size:0.85rem;">No assignments this week yet.</p>';
            return;
        }
        ...
        container.innerHTML = sorted.map(([key, count]) => {
            ...
            const bars = '█'.repeat(count) + '░'.repeat(Math.max(0, maxCount - count));
            return `
    ...
    window.openShiftModal = async function(date, slot, startTime, endTime) {
        ...
        if (infoEl) {
            const meta = GLOBAL_SHIFTS.find(s => s.id === slot) || { label: slot, icon: 'fa-solid fa-clock', color: '#94A3B8' };
            infoEl.innerHTML = `
        ...
        if (sel) {
            ...
            if (window.EMPLOYEES_CACHE && window.EMPLOYEES_CACHE.length > 0) {
                ...
                window.EMPLOYEES_CACHE.forEach(e => {
                    const opt = document.createElement('option');
                    opt.value = e.id;
                    const optName = `${e.first_name || ''} ${e.last_name || ''}`.trim() || e.username || e.email || `Staff ${e.id}`;
                    opt.textContent = `${optName} (${e.role || 'Employee'})`;
                    sel.appendChild(opt);
                });
            } else {
                sel.innerHTML = '<option value="" disabled selected>Loading employees...</option>';
            }
        }
        try {
        ...
        if (ttEmployees.length === 0 && window.EMPLOYEES_CACHE && window.EMPLOYEES_CACHE.length > 0) {
        ...
        if (sel) {
            ...
            if (ttEmployees.length === 0) {
                sel.innerHTML += '<option disabled>No active employees found — ensure employees are added in HR tab</option>';
            } else {
    ...
    async function fetchCurrentAssignees(date, slot) {
        ...
        try {
            const res  = await fetchAuth(`${API_URL}/shift-assignments?from=${date}&to=${date}`);
            const data = await res.json();
            const mine = (data.assignments || []).filter(a => a.slot === slot);

            if (mine.length > 0) {
                listDiv.style.display = 'block';
                chipsDiv.innerHTML = mine.map(a => `
    ...
    window.removeShiftAssignment = async function(id) {
        ...
        try {
            const res = await fetchAuth(`${API_URL}/shift-assignments/${id}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    window.smartAutoSchedule = async function() {
        const from = dateStr(ttWeekStart);
        if (!confirm(`Auto-schedule the week of ${from}?\nThis will replace any existing assignments for this week.`)) return;
        ...
        try {
            ...
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Auto-Schedule Week'; }
        }
    };
    ...
    window.clearShiftWeek = async function() {
        const from = dateStr(ttWeekStart);
        if (!confirm(`Clear ALL shift assignments for the week of ${from}?`)) return;
        try {
            const res = await fetchAuth(`${API_URL}/shift-assignments/week/${from}`, { method: 'DELETE' });
            const d   = await res.json();
        ...
        if (window._socket) {
            window._socket.on('db_updated', (data) => {
                if (data.module === 'shift_assignments') {
                    ...
                    if (timetableSection && timetableSection.classList.contains('active')) {
                        renderTimetable();
                    }
                }
            });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // Intercept nav button for 'schedules' tab
        document.querySelectorAll('.nav-btn[data-target="schedules"]').forEach(btn => {
            btn.addEventListener('click', () => {
                ttWeekStart = getMondayOfWeek(new Date());
                loadShiftTimetable();
    ...
    window.loadDayTimetable = async function() {
        ...
        try {
            const res  = await fetchAuth(`${API_URL}/shift-assignments?from=${today}&to=${today}`);
            const data = await res.json();
            const rows = data.assignments || [];

            tbody.innerHTML = '';
            if (rows.length === 0) {
                tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:20px;color:#94A3B8;">No shifts scheduled today.</td></tr>';
                return;
            }
            rows.forEach(r => {
                const meta = GLOBAL_SHIFTS.find(s => s.id === r.slot) || { label: r.slot, icon: 'fa-solid fa-clock', color: '#94A3B8' };
                const now  = new Date();
...
window.loadPendingCOD = async function() {
    try {
        ...
        if (res.ok) {
            ...
            if (listContainer) {
                if (deliveries.length === 0) {
                    listContainer.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;">No pending Cash on Delivery orders.</p>';
                    return;
                }
...
window.markCODReceived = async function(posOrderId) {
    if (!confirm('Are you sure you have received the cash for this order?')) return;
...
async function loadTechUsers() {
    try {
        ...
        if (!data.tech_users || data.tech_users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; padding:20px; color:var(--text-muted);">No tech accounts found.</td></tr>';
            return;
        }
...
async function deleteTechUser(id, username) {
    if (!confirm(`Are you sure you want to delete tech account '${username}'?`)) return;
    try {
        const res = await fetchAuth(`${API_URL}/system/tech_users/${id}`, { method: 'DELETE' });
        const data = await res.json();
...
async function toggleCompanyStatus(prefix, currentStatus) {
    ...
    if (!confirm(`Are you sure you want to ${newStatus === 'PAUSED' ? 'pause' : 'resume'} the company ${prefix}?`)) return;
...
async function deleteCompany(prefix) {
    if (!confirm(`WARNING: You are about to PERMANENTLY DELETE the company ${prefix} and all its data. This action cannot be undone. Are you absolutely sure?`)) return;
...
function openNewCompanyModal() {
    ...
    prefixInput.oninput = function() {
        ...
        this.value = val;
        document.getElementById('new-company-preview').textContent = val ? `${val}00001` : 'XXX00001';
    };
}

async function submitNewCompany() {
    const name = document.getElementById('new-company-name').value.trim();
    const prefix = document.getElementById('new-company-prefix').value.trim().toUpperCase();
    ...
    if (!name || !prefix) {
        ...
        resultEl.textContent = 'Company name and prefix are required.';
        return;
    }
    ...
    try {
        const res = await fetch(`${API_URL}/system/initialize`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('jomish_token')}` },
            body: JSON.stringify({ company_name: name, company_prefix: prefix, tech_password: techPass })
        });
        const data = await res.json();
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.nav-btn[data-target="tech-hub"]').forEach(btn => {
        btn.addEventListener('click', () => { 
            setTimeout(() => {
                loadCompanies();
                loadTechUsers();
...
async function syncOfflineMutations() {
    ...
    try {
        ...
        if (mutations.length > 0) {
            console.log(`[Offline Sync] Found ${mutations.length} pending mutations to sync.`);
            showToast('Syncing offline changes...', 'info');
            for (const m of mutations) {
                try {
                    console.log(`[Offline Sync] Replaying ${m.method} ${m.url}`);
                    ...
                    if (res.ok) {
                        ...
                        console.error(`[Offline Sync] Failed to replay mutation ${m.id} - Status: ${res.status}`);
                    ...
                    console.error(`[Offline Sync] Network error while replaying ${m.id}:`, e);
                    // Network error, stop syncing for now, retry later.
                    break;
                }
            }
            showToast('Offline changes synced successfully.', 'success');
            // Refresh current view if applicable to fetch updated IDs
            if (typeof loadEmployees === 'function' && currentView === 'hr') loadEmployees();
        }
    } catch (e) {
...
async function subscribeToPush(registration) {
    try {
        if (!window.USER_ID) return; // Only subscribe if logged in
...
// ==========================================
// WhatsApp Integration (Secretary Hub)
// ==========================================

/**
 * Fetches current WhatsApp status from the server REST endpoint and updates
 * the UI accordingly.  Called every time the user switches to the WhatsApp
 * view so that the QR code is shown even if the socket event fired before
 * the browser connected.
 */
async function loadWhatsAppStatus() {
    try {
        const status = await fetchAuth('/api/whatsapp/status');
        if (!status) return;

        const loadingEl = document.getElementById('whatsapp-loading');
        const qrImg     = document.getElementById('whatsapp-qr-img');
        const notReady  = document.getElementById('whatsapp-not-ready');
        const ready     = document.getElementById('whatsapp-ready');
        if (!loadingEl || !qrImg || !notReady || !ready) return;
        ...
        if (status.ready) {
            ...
            // QR exists on the server — generate a data-URL from it
            try {
                // Use the /api/whatsapp/qr endpoint if available, otherwise
                // fall back to asking the server to regenerate via socket.
                ...
                // /api/whatsapp/qr not yet available — show spinner and wait
                // for socket event
                loadingEl.style.display = 'block';
                qrImg.style.display = 'none';
            }
        } else {
            // Not ready and no QR yet — keep spinner
            loadingEl.style.display = 'block';
            qrImg.style.display = 'none';
            notReady.style.display = 'flex';
            ready.style.display = 'none';
        ...
        console.error('loadWhatsAppStatus error', err);
    }
}

// Real-time updates via socket (fires for NEW QR codes while the page is open)
if (window.socket) {
    window.socket.on('whatsapp_qr', (qrDataUrl) => {
        const loadingEl = document.getElementById('whatsapp-loading');
        const qrImg     = document.getElementById('whatsapp-qr-img');
        const notReady  = document.getElementById('whatsapp-not-ready');
        const ready     = document.getElementById('whatsapp-ready');
    ...
    window.socket.on('whatsapp_ready', () => {
        const notReady = document.getElementById('whatsapp-not-ready');
        const ready    = document.getElementById('whatsapp-ready');
    ...
    window.socket.on('whatsapp_disconnected', () => {
        const notReady  = document.getElementById('whatsapp-not-ready');
        const ready     = document.getElementById('whatsapp-ready');
        const qrImg     = document.getElementById('whatsapp-qr-img');
        const loadingEl = document.getElementById('whatsapp-loading');
    ...
    window.socket.on('whatsapp_message', (msg) => {
        const inbox = document.getElementById('whatsapp-inbox');
        if (!inbox) return;
        ...
        msgDiv.innerHTML = `
            <div style="font-size:0.8rem; color:#94A3B8; margin-bottom:4px;">From: ${msg.from}</div>
...
document.addEventListener('DOMContentLoaded', () => {
    const waForm = document.getElementById('form-whatsapp-send');
    if (waForm) {
        waForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetchAuth('/api/whatsapp/send', {
                    method: 'POST',
...
// ===== JomishPOS: Toggle between POS and SME Finance =====
function toggleSmeFinanceMode() {
    const pos = document.getElementById('pos-terminal');
    const sme = document.getElementById('sme-business');
    if (!pos || !sme) return;
