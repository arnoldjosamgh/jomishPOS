...
const { buildBuffer, buildReceiptLines, receiptTextToLines, printViaNetwork } = require('./ThermalPrinter');
const { initWhatsApp, getWhatsAppStatus, getWhatsAppQR, sendWhatsAppMessage } = require('./whatsapp');
...
app.get('/api/whatsapp/qr', async (req, res) => {
    ...
    if (!qrDataUrl) return res.status(404).json({ error: 'No QR code available' });
    res.json({ qrDataUrl });
});
...
const APP_BASE_DIR = process.pkg ? process.cwd() : path.resolve(__dirname, '..');

// Load Config
let config = { port: 3005, mDNS_name: 'business-system' };
try {
    const configData = fs.readFileSync(path.join(APP_BASE_DIR, 'config/config.json'));
...
function getCache(schema, key) {
    ...
    if (Date.now() > entry.exp) { _cache.delete(`${schema}:${key}`); return null; }
    return entry.val;
}
...
function rateLimit(key, maxAttempts, windowMs) {
    const now = Date.now();
    if (!rateLimiters[key]) rateLimiters[key] = { count: 0, resetAt: now + windowMs };
...
setInterval(() => {
    const now = Date.now();
    for (const k in rateLimiters) { if (now > rateLimiters[k].resetAt) delete rateLimiters[k]; }
}, 10 * 60 * 1000);
...
app.use((req, res, next) => {
    ...
    if (req.path === '/api/login' && req.body.username) {
        ...
        if (username.toLowerCase() === 'tech' || username.toLowerCase() === 'jomish_tech') {
            prefix = 'public';
        } else if (match) {
            prefix = match[1];
        } else if (tecMatch) {
    ...
    const schemaName = prefix === 'public' ? 'public' : ('t_' + prefix.toLowerCase());
...
app.get('/api/employees', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT id, first_name, last_name, email, username, role, department, salary, employee_code, photo_base64, profile_color, layout_type, created_at, is_active, COALESCE(is_suspended, 0) as is_suspended,
            ...
            FROM employees WHERE is_active = 1`, [], (err, rows) => {
...
app.post('/api/login/demo', (req, res) => {
    ...
    const permissions = {
        can_see_dashboard: 1, can_see_hr: 1, can_see_attendance: 1,
        can_see_sme: 1, can_see_pos: 1, can_see_secretary: 1, can_see_transport: 1
    };
...
app.post('/api/print-receipt', authenticateToken, (req, res) => {
    ...
    const printer = new ThermalPrinter((buffer) => {
        ...
        console.log('[Printer Output]', buffer);
    });
    ...
    db.get('SELECT password FROM employees WHERE id = ?', [req.user.id], async (err, user) => {
...
app.post('/api/login', (req, res) => {
    ...
    if (prefix.toUpperCase() !== 'PUBLIC' && prefix.toUpperCase() !== 'DEMO') {
        return new Promise((resolve) => {
            asyncLocalStorage.run('public', () => {
                db.get('SELECT status FROM companies WHERE prefix = ?', [prefix.toUpperCase()], (err, row) => {
                    resolve(row ? row.status : null);
                });
            });
        }).then(status => {
            if (status === 'PAUSED') {
                return res.status(403).json({ error: 'Company account is temporarily paused. Please contact support.' });
            }
            continueLogin();
        });
    } else {
    ...
    function continueLogin() {
        const proceedWithTenantLogin = () => {
            ...
            const schemaName = (prefix === 'PUBLIC' || prefix === 'DEMO')
                ? prefix.toLowerCase()
            ...
            const numericId = /^\d+$/.test(actualUsername) ? parseInt(actualUsername) : null;
            ...
            db.get(query, params, async (err, user) => {
                ...
                if (!user && numericPart) {
                    ...
                    db.get(
                        ...
                        async (err2, user2) => {
                            if (err2 || !user2) return res.status(401).json({ error: 'Invalid credentials' });
                ...
                async function finalizeLogin(u) {
                    ...
                    if (u.is_active === 0) {
                        console.warn(`[LOGIN BLOCKED] Terminated employee — ID: ${u.id}, Name: ${u.first_name} ${u.last_name}`);
                    ...
                    if (u.is_suspended === 1) {
                        console.warn(`[LOGIN BLOCKED] Suspended employee — ID: ${u.id}, Name: ${u.first_name} ${u.last_name}`);
                    ...
                    const token = jwt.sign(
                        { id: u.id, role: u.role, name: `${u.first_name} ${u.last_name}`, permissions, prefix },
                        JWT_SECRET,
                        { expiresIn: '8h' }
                    );
                    res.cookie('jomish_auth', token, { httpOnly: true, secure: false, sameSite: 'lax', maxAge: 8 * 60 * 60 * 1000 });
                    res.json({ token, role: u.role, name: `${u.first_name} ${u.last_name}`, permissions, user_id: u.id, prefix });
                }
            });
        ...
        asyncLocalStorage.run('public', () => {
            db.get('SELECT * FROM tech_users WHERE username = ?', [username], async (err, techUser) => {
                let isTech = false;
                ...
                } else if ((tecMatch && password === 'Jomish9!!') || 
...
app.patch('/api/system/companies/:prefix/status', authenticateToken, (req, res) => {
    if (req.user.name !== 'System Technician') return res.status(403).json({ error: 'Forbidden' });
    const { status } = req.body;
    if (status !== 'ACTIVE' && status !== 'PAUSED') return res.status(400).json({ error: 'Invalid status' });
    asyncLocalStorage.run('public', () => {
        db.run('UPDATE companies SET status = ? WHERE prefix = ?', [status, req.params.prefix.toUpperCase()], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ message: `Company ${req.params.prefix} is now ${status}` });
        });
    });
});
...
app.post('/api/system/initialize', async (req, res) => {
    ...
    if (!company_prefix || !company_name) {
        return res.status(400).json({ error: 'Company prefix and name are required.' });
    }

    // Sanitize prefix: uppercase, letters only, max 5 chars
    const prefix = company_prefix.toUpperCase().replace(/[^A-Z]/g, '').substring(0, 5);
    if (!prefix) return res.status(400).json({ error: 'Invalid company prefix. Use letters only.' });
        ...
        await new Promise((resolve, reject) => {
            asyncLocalStorage.run('public', () => {
                db.run(`CREATE TABLE IF NOT EXISTS companies (
                    ...
                    db.run(`INSERT INTO companies (prefix, name) VALUES (?, ?)`, [prefix, company_name], (err) => {
        ...
        asyncLocalStorage.run(schemaName, async () => {
            ...
            db.run(
                `INSERT INTO employees (first_name, last_name, email, username, role, department, salary, password, employee_code, is_active)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                ['HR', 'Admin', `${techUserId.toLowerCase()}@${prefix.toLowerCase()}.local`, techUserId, 'HR', 'Administration', 0, hashedPassword, techUserId],
                function(insertErr) {
                    ...
                    const upsertSetting = (key, value) => {
                        db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES (?, ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [key, value]);
                    };
                    ...
                    console.log(`[INIT] Company "${company_name}" initialized. First user: ${techUserId}`);
                    res.json({
...
app.post('/api/employees', authenticateToken, async (req, res) => {
    if (req.user.role !== 'HR' && req.user.role !== 'CEO') {
        return res.status(403).json({ error: 'Forbidden: Only HR or CEO can add employees.' });
    }
    const { first_name, last_name, email, role, department, salary, password, photo_base64, profile_color, layout_type } = req.body;

    if (!password) return res.status(400).json({ error: 'Initial login password is required.' });
                ...
                function(err) {
                    if (err) {
                        ...
                        if ((err.message || '').includes('UNIQUE constraint failed') || (err.message || '').includes('duplicate key value violates unique constraint')) {
                            if ((err.message || '').includes('username') || (err.message || '').includes('employee_code')) {
                                return res.status(400).json({ error: 'System error: Employee ID counter out of sync. Please try adding the employee again.' });
                            }
                            return res.status(400).json({ error: 'This email is already registered to another staff member.' });
                        }
                        return res.status(500).json({ error: err.message });
                    }
                    if (role) {
                        db.run('INSERT INTO roles_config (role_name) VALUES (?) ON CONFLICT DO NOTHING', [role]);
                    }
                    ...
                    db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('next_employee_number', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = EXCLUDED.setting_value`, [String(num + 1)]);
...
app.patch('/api/employees/:id/permissions', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR') return res.status(403).json({ error: 'Forbidden' });
    const { id } = req.params;
    ...
    db.run(`UPDATE employees SET ${setClauses.join(', ')} WHERE id = ?`, values, function(err) {
        if (err) {
...
app.delete('/api/employees/:id', authenticateToken, (req, res) => {
    ...
    db.run('UPDATE employees SET is_active = 0 WHERE id = ?', [id], function(err) {
        ...
        db.run('UPDATE employees SET username = NULL, password = NULL, email = email || "-revoked-" || id WHERE id = ?', [id], function(credErr) {
            if (credErr) {
                // Log but don't block the response; deactivation already succeeded
                console.error(`[TERMINATION] Failed to revoke credentials for employee #${id}:`, credErr.message);
            } else {
                console.log(`[TERMINATION] Login credentials revoked for employee #${id}.`);
            }
            emitAndBust('employees', 'employees');
            res.json({ message: 'Employee terminated, ID revoked, and login access permanently removed.' });
        });
    });
});
...
app.patch('/api/employees/:id/suspend', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR') return res.status(403).json({ error: 'Forbidden' });
    db.run('UPDATE employees SET is_suspended = 1 WHERE id = ? AND is_active = 1', [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        emitAndBust('employees', 'employees');
        res.json({ message: 'Account suspended. Employee cannot log in until restored.' });
    });
});
...
app.patch('/api/employees/:id/unsuspend', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR') return res.status(403).json({ error: 'Forbidden' });
    db.run('UPDATE employees SET is_suspended = 0 WHERE id = ? AND is_active = 1', [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        emitAndBust('employees', 'employees');
...
app.put('/api/roles/:name', authenticateToken, (req, res) => {
    ...
    const { can_see_dashboard, can_see_hr, can_see_attendance, can_see_sme, can_see_pos, can_see_secretary, can_see_transport, can_see_hardware, can_see_system_users, can_see_schedules } = req.body;
    ...
    `, [name, can_see_dashboard, can_see_hr, can_see_attendance, can_see_sme, can_see_pos, can_see_secretary || 0, can_see_transport || 0, can_see_hardware || 0, can_see_system_users || 0, can_see_schedules || 0], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        ...
        db.run(`UPDATE employees SET 
                ...
                WHERE role = ?`, 
                [can_see_dashboard, can_see_hr, can_see_attendance, can_see_sme, can_see_pos, 
                 can_see_secretary || 0, can_see_transport || 0, can_see_hardware || 0, can_see_system_users || 0, can_see_schedules || 0, 
                 name], 
        function(err2) {
            emitAndBust('roles', 'roles');
            bustCache(getSchema(), 'employees');
            res.json({ message: 'Role limits updated and applied to existing staff.' });
        });
    });
});
...
app.patch('/api/roles/:name/password', authenticateToken, async (req, res) => {
    ...
    const { name } = req.params;
    const { password } = req.body;
    if (!password || password.trim().length < 1) return res.status(400).json({ error: 'Password required' });
    try {
        const hashed = await bcrypt.hash(password, 10);
        db.run('UPDATE employees SET password = ? WHERE role = ?', [hashed, name], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            emitAndBust('employees', 'employees');
            res.json({ message: `Password set for ${this.changes} user(s) with role '${name}'`, updated: this.changes });
        });
    } catch(e) {
...
app.delete('/api/roles/:roleName', authenticateToken, (req, res) => {
    ...
    db.run('DELETE FROM roles_config WHERE role_name = ?', [roleName], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        emitAndBust('roles', 'roles');
...
app.post('/api/scan/global', authenticateToken, (req, res) => {
    ...
    db.get('SELECT * FROM employees WHERE id = ? OR employee_code = ? OR id = ?', [code, code, idOnly], (err, emp) => {
        if (err) return res.status(500).json({ error: err.message });
        if (emp) {
            if (emp.is_active === 0) return res.status(403).json({ error: 'Staff account is inactive.' });
            ...
            db.run('INSERT INTO attendance_logs (employee_id, scan_time, scan_type, status) VALUES (?, ?, ?, ?)',
                [emp.id, localTime, scanType, 'SUCCESS'], function(err) {
                    if (err) return res.status(500).json({ error: err.message });
                    db.run('UPDATE employees SET is_present = ? WHERE id = ?', [newStatus, emp.id], (err) => {
...
app.post('/api/attendance/manual-out', authenticateToken, (req, res) => {
    ...
    db.get('SELECT first_name FROM employees WHERE id = ?', [employee_id], (err, row) => {
        ...
        db.run('INSERT INTO attendance_logs (employee_id, scan_time, scan_type, status) VALUES (?, ?, ?, ?)',
            ...
            function(err) {
                if (err) return res.status(500).json({ error: err.message });
                db.run('UPDATE employees SET is_present = 0 WHERE id = ?', [employee_id], (err) => {
...
app.post('/api/transactions', authenticateToken, (req, res) => {
    ...
    const dateToRecord = transaction_date ? new Date(transaction_date).toISOString() : new Date().toISOString();
    db.run(
...
app.get('/api/system/autostart', authenticateToken, (req, res) => {
    if (req.user.name !== 'System Technician') return res.status(403).json({ error: 'Forbidden' });
    const startupPath = path.join(process.env.APPDATA || '', 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup', 'JomishSuite.lnk');
...
app.get('/api/system/tech_users', authenticateToken, (req, res) => {
    ...
    asyncLocalStorage.run('public', () => {
        db.all('SELECT id, username, created_at FROM tech_users', [], (err, rows) => {
            if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/system/tech_users', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        asyncLocalStorage.run('public', () => {
            db.run('INSERT INTO tech_users (username, password) VALUES (?, ?)', [username, hash], function(err) {
                if (err) {
...
app.delete('/api/system/tech_users/:id', authenticateToken, (req, res) => {
    if (req.user.id !== 9999) return res.status(403).json({ error: 'Forbidden' });
    asyncLocalStorage.run('public', () => {
        db.get('SELECT username FROM tech_users WHERE id = ?', [req.params.id], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
            ...
            if (row.username === 'tech') {
                return res.status(400).json({ error: 'Cannot delete the primary tech account' });
            }
            db.run('DELETE FROM tech_users WHERE id = ?', [req.params.id], (err) => {
                if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/system/autostart', authenticateToken, (req, res) => {
    ...
    exec(`powershell.exe -NoProfile -Command "${psCommand}"`, (error) => {
        ...
        res.json({ message: enable ? 'Auto-start enabled.' : 'Auto-start disabled.' });
    });
});

app.post('/api/system/create-shortcut', authenticateToken, (req, res) => {
    if (req.user.name !== 'System Technician') return res.status(403).json({ error: 'Forbidden' });
    const desktopPath = path.join(process.env.USERPROFILE || process.env.HOME || '', 'Desktop', 'Jomish Suite.lnk');
    const targetScript = path.resolve(APP_BASE_DIR, 'Start_Jomish_Suite.vbs');
...
app.post('/api/system/set-static-ip', authenticateToken, (req, res) => {
    ...
    exec(`powershell.exe -NoProfile -ExecutionPolicy Bypass -File "${scriptPath}"`, (error) => {
        if (error) {
    ...
    res.json({ message: 'Static IP Configuration script launched! Please check for the administrator prompt.' });
});

app.post('/api/system/print-raw', authenticateToken, (req, res) => {
    const { receiptText, receiptData, printerName } = req.body;
    if (!receiptText && !receiptData) return res.status(400).json({ error: 'No receipt data provided' });

    const platform = os.platform();

    try {
        // Build ESC/POS binary buffer using the encoder library
        const lines = receiptData ? buildReceiptLines(receiptData) : receiptTextToLines(receiptText);
        ...
        if (platform === 'win32') {
            ...
            const parallelExists = (() => { try { fs.accessSync(parallelPrinter, fs.constants.W_OK); return true; } catch(e) { return false; } })();
            const usbExists = (() => { try { fs.accessSync(usbPrinter, fs.constants.W_OK); return true; } catch(e) { return false; } })();
        ...
        exec(printCmd, { timeout: 10000, shell: platform === 'win32' ? undefined : '/bin/sh' }, (err) => {
            try { fs.unlinkSync(tempFile); } catch (e) {}
            if (err) {
                console.error('[PRINT ERROR]', err.message);
                return res.status(500).json({ error: 'Failed to print: ' + err.message });
            }
...
app.post('/api/system/print-network', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        console.log(`[PRINT] Sending ${buffer.length} bytes to ${printerIp}:${printerPort} via TCP`);
        await printViaNetwork(buffer, printerIp, Number(printerPort));
        console.log(`[PRINT] Successfully sent to ${printerIp}:${printerPort}`);
        res.json({ success: true, message: `Sent to printer at ${printerIp}:${printerPort}` });
    } catch (err) {
...
app.post('/api/system/reset', authenticateToken, (req, res) => {
    if (req.user.name !== 'System Technician') {
        return res.status(403).json({ error: 'Access Denied: Only the System Technician can perform a system reset.' });
    }
    ...
    db.serialize(() => {
        ...
        const tablesToClear = [
            'transactions', 'pos_orders', 'sold_barcodes', 'attendance_logs', 
            'schedules', 'employee_notes', 'internal_messages', 'email_messages', 
            'notices', 'calendar_events', 'shifts', 'devices', 'products', 'employees',
            'credit_records', 'expense_categories'
        ];
        ...
        const checkCompletion = () => {
            ...
            if (completedQueries === tablesToClear.length) {
                ...
                bcrypt.hash('ceo123', saltRounds, (err, ceoHash) => {
                    if (err) {
                        db.run('ROLLBACK');
                        return res.status(500).json({ error: 'Failed to hash CEO password: ' + err.message });
                    }
                    bcrypt.hash('admin123', saltRounds, (err, hrHash) => {
                        if (err) {
                            db.run('ROLLBACK');
                            return res.status(500).json({ error: 'Failed to hash HR password: ' + err.message });
                        }
                        ...
                        db.run(
                            ...
                            function(err) {
                                if (err) {
                                    db.run('ROLLBACK');
                                    return res.status(500).json({ error: 'Failed to seed CEO: ' + err.message });
                                }
                                ...
                                db.run(
                                    ...
                                    function(err) {
                                        if (err) {
                                            db.run('ROLLBACK');
                                            return res.status(500).json({ error: 'Failed to seed HR: ' + err.message });
                                        }
        ...
        tablesToClear.forEach(table => {
            db.run(`DELETE FROM ${table}`, [], function(err) {
                if (err) {
                    errorOccurred = true;
                    db.run('ROLLBACK');
                    return res.status(500).json({ error: `Failed to clear table ${table}: ` + err.message });
                }
                completedQueries++;
                checkCompletion();
            });
...
app.get('/api/system/backup', authenticateToken, (req, res) => {
    if (req.user.name !== 'System Technician') {
        return res.status(403).json({ error: 'Access Denied: Only the System Technician can perform a backup.' });
    }
    ...
    if (!dbFilePath) {
        return res.status(400).json({ error: 'Backup is only supported with SQLite databases.' });
    }
    ...
    db.run('PRAGMA wal_checkpoint(TRUNCATE)', (err) => {
        ...
        fileStream.pipe(res);
        console.log(`[BACKUP] Database backup downloaded by ${req.user.name}`);
    });
});
...
app.post('/api/system/restore', authenticateToken, (req, res) => {
    if (req.user.name !== 'System Technician') {
        return res.status(403).json({ error: 'Access Denied: Only the System Technician can perform a restore.' });
    }
    ...
    if (!dbFilePath) {
        return res.status(400).json({ error: 'Restore is only supported with SQLite databases.' });
    }
    ...
    req.on('end', () => {
        ...
        if (buffer.length < 100) {
            return res.status(400).json({ error: 'Uploaded file appears to be empty or corrupt.' });
        }
        ...
        try {
            ...
            console.log(`[RESTORE] Backup staged to ${restorePath} by ${req.user.name}. Restarting server to apply...`);
            ...
            res.status(500).json({ error: 'Failed to write restored database: ' + e.message });
        }
    });
...
app.post('/api/system/update', authenticateToken, (req, res) => {
    if (req.user.name !== 'System Technician') {
        return res.status(403).json({ error: 'Access Denied: Only the System Technician can perform an update.' });
    }
    ...
    req.on('end', () => {
        ...
        try {
            ...
            console.log(`[UPDATE] Extracting update package staged at ${zipPath}...`);
            
            // Extract using PowerShell
            exec(`powershell -Command "Expand-Archive -Path '${zipPath}' -DestinationPath '${stagingDir}' -Force"`, (err) => {
                if (err) {
...
`;
                ...
                console.log(`[UPDATE] Update staged successfully by ${req.user.name}. Restarting server to apply...`);
                res.json({ success: true, message: 'Update staged successfully. Server is restarting.' });
...
app.get('/api/products', authenticateToken, (req, res) => {
    ...
    db.all('SELECT * FROM products', [], (err, products) => {
        if (err) return res.status(500).json({ error: err.message });
        db.all('SELECT product_id, barcode FROM product_barcodes', [], (err, barcodes) => {
            if (err) {
                return res.json({ products: products.map(p => ({ ...p, additional_barcodes: [] })) });
            }
            const barcodeMap = {};
            barcodes.forEach(b => {
...
app.post('/api/products', authenticateToken, (req, res) => {
    ...
    db.run('INSERT INTO products (name, category, price, stock, barcode, barcode_end, photo_base64, buying_price) VALUES (?, ?, ?, ?, NULL, NULL, ?, ?)',
        [name, category, price, stock, photo_base64 || null, bp], function(err) {
        if (err) {
...
app.post('/api/products/:id/scan-assign', authenticateToken, (req, res) => {
    ...
    db.get(`SELECT id FROM product_barcodes WHERE product_id = ? AND barcode = ?`, [productId, barcode], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        ...
        db.run(`INSERT INTO product_barcodes (product_id, barcode, source) VALUES (?, ?, 'scanned')`, [productId, barcode], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            db.run(`UPDATE products SET stock = stock + ? WHERE id = ?`, [qty, productId], () => {
                emitAndBust('products', 'products');
...
app.post('/api/products/:id/generate-label', authenticateToken, (req, res) => {
    ...
    db.get(`SELECT id FROM product_barcodes WHERE product_id = ? AND barcode = ?`, [productId, label], (err, existing) => {
        if (err) return res.status(500).json({ error: err.message });
        if (existing) {
            // Already registered - just add stock
            db.run(`UPDATE products SET stock = stock + 1 WHERE id = ?`, [productId], () => {
                emitAndBust('products', 'products');
                res.json({ success: true, barcode: label, note: 'Shared barcode reused' });
            });
        } else {
            db.run(`INSERT INTO product_barcodes (product_id, barcode, source) VALUES (?, ?, 'generated')`, [productId, label], function(err) {
                if (err) return res.status(500).json({ error: err.message });
                db.run(`UPDATE products SET stock = stock + 1 WHERE id = ?`, [productId], () => {
                    emitAndBust('products', 'products');
...
app.post('/api/products/:id/generate-labels-bulk', authenticateToken, (req, res) => {
    const productId = req.params.id;
    ...
    db.get(`SELECT id FROM product_barcodes WHERE product_id = ? AND barcode = ?`, [productId, sharedBarcode], (err, existing) => {
        if (err) return res.status(500).json({ error: err.message });

        if (existing) {
            // Barcode already registered â€” just increase stock
            db.run(`UPDATE products SET stock = stock + ? WHERE id = ?`, [count, productId], (err) => {
                if (err) return res.status(500).json({ error: err.message });
                emitAndBust('products', 'products');
                res.json({ success: true, count, labels: [sharedBarcode], note: 'Shared barcode reused' });
            });
        } else {
            // Register the shared barcode once, then increase stock
            db.run(`INSERT INTO product_barcodes (product_id, barcode, source) VALUES (?, ?, 'generated')`, [productId, sharedBarcode], (err) => {
                if (err) return res.status(500).json({ error: err.message });
                db.run(`UPDATE products SET stock = stock + ? WHERE id = ?`, [count, productId], (err) => {
                    if (err) return res.status(500).json({ error: err.message });
                    emitAndBust('products', 'products');
...
app.patch('/api/products/:id/stock', authenticateToken, (req, res) => {
    ...
    if (inc <= 0) return res.status(400).json({ error: 'Increment must be positive' });
    ...
    db.get('SELECT * FROM products WHERE id = ?', [productId], (err, product) => {
        ...
        const oldEnd = parseInt(product.barcode_end) || (parseInt(product.barcode) || 0);
        const newLabelStart = oldEnd + 1;
        const newLabelEnd = oldEnd + inc;

        db.run('UPDATE products SET stock = stock + ?, barcode_end = ? WHERE id = ?',
            [inc, newLabelEnd.toString(), productId], function(e) {
            ...
            res.json({
                ...
                product: { ...product, stock: product.stock + inc, barcode_end: newLabelEnd.toString() }
            });
        });
    });
});
...
app.put('/api/products/:id', authenticateToken, (req, res) => {
    let { name, category, price, barcode, barcode_end, photo_base64, buying_price } = req.body;
    const { id } = req.params;
    ...
    let sql = 'UPDATE products SET name = ?, category = ?, price = ?, barcode = ?, barcode_end = ?, buying_price = ? WHERE id = ?';

        if (photo_base64 && photo_base64.length > 50) {
            sql = 'UPDATE products SET name = ?, category = ?, price = ?, barcode = ?, barcode_end = ?, buying_price = ?, photo_base64 = ? WHERE id = ?';
...
app.patch('/api/products/:id/photo', authenticateToken, (req, res) => {
    ...
    console.log(`[PRODUCT PHOTO UPDATE] ID: ${req.params.id}, Has Photo: ${photo_base64 ? 'YES (' + photo_base64.length + ' chars)' : 'NO'}`);
    db.run('UPDATE products SET photo_base64 = ? WHERE id = ?', [photo_base64, req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        emitAndBust('products', 'products');
...
app.post('/api/checkout', authenticateToken, (req, res) => {
    const { total_amount, items, payment_method = 'CASH', amount_paid, buyer_name, buyer_phone, promised_date, is_delivery, delivery_address, driver_name } = req.body;
    ...
    db.serialize(() => {
        ...
        const txAmount = (payment_method === 'COD' || payment_method === 'INVOICE') ? total_amount : paid;

        db.run('INSERT INTO transactions (amount, type, description, recorded_by, transaction_date, payment_status) VALUES (?, ?, ?, ?, ?, ?)',
            [txAmount, 'INCOME', description, cashier_id, now, payment_status], function(err) {
                if (err) {
                    db.run('ROLLBACK');
                ...
                db.run('INSERT INTO pos_orders (cashier_id, total_amount, transaction_id, payment_method, amount_paid) VALUES (?, ?, ?, ?, ?)',
                    [cashier_id, total_amount, transaction_id, effectivePaymentMethod, paid], function(err2) {
                        if (err2) {
                            db.run('ROLLBACK');
                            return res.status(500).json({ error: 'Order failed: ' + err2.message });
                        }
                            ...
                            items.forEach(item => {
                                ...
                                if (item.barcodes && Array.isArray(item.barcodes)) {
                                    item.barcodes.forEach(bc => {
                                        db.run('INSERT OR IGNORE INTO sold_barcodes (barcode, product_id, transaction_id) VALUES (?, ?, ?)',
                            ...
                            db.run('COMMIT', (err3) => {
                                ...
                                items.forEach(item => {
                                    ...
                                    db.run(
                                        ...
                                        (errInsert) => {
                                            if (errInsert) console.error('Failed to insert order_item:', errInsert.message);
                                        }
                                    );
                                });
                        ...
                        const handleDeliveryAndComplete = () => {
                            ...
                            if (effectiveIsDelivery) {
                                db.run('INSERT INTO deliveries (order_id, client_name, client_phone, client_location, status, driver_name) VALUES (?, ?, ?, ?, ?, ?)',
                                    [pos_order_id, buyer_name || 'Customer', buyer_phone || '', delivery_address || '', 'Pending', driver_name || null], function(errDel) {
                                        if (errDel) {
                                            db.run('ROLLBACK');
                                            return res.status(500).json({ error: 'Delivery record failed: ' + errDel.message });
                                        }
                                        completeCheckout();
                                    });
                            } else {
                        ...
                        if (payment_method === 'CREDIT' && paid < total_amount) {
                            ...
                            db.run('INSERT INTO credit_records (buyer_name, buyer_phone, pos_order_id, total_amount, amount_paid, balance, promised_date) VALUES (?, ?, ?, ?, ?, ?, ?)',
                                [buyer_name || 'Unknown', buyer_phone || '', pos_order_id, total_amount, paid, balance, promised_date || null], function(errCredit) {
                                    if (errCredit) {
                                        db.run('ROLLBACK');
                                        return res.status(500).json({ error: 'Credit record failed: ' + errCredit.message });
                                    }
...
app.get('/api/pos_orders/tx/:transaction_id', authenticateToken, (req, res) => {
    ...
    `, [txId], (err, order) => {
        ...
        db.all(`SELECT * FROM order_items WHERE pos_order_id = ?`, [order.id], (err2, items) => {
...
app.post('/api/credits/pay', authenticateToken, (req, res) => {
    ...
    if (!buyer_name || !amount_paid || Number(amount_paid) <= 0) {
        return res.status(400).json({ error: 'Buyer name and a valid payment amount are required.' });
    }
    ...
    db.get(
        ...
        (err, row) => {
            ...
            if (paid > totalBalance) {
                return res.status(400).json({ error: `Payment of UGX ${paid.toLocaleString()} exceeds outstanding balance of UGX ${totalBalance.toLocaleString()}.` });
            }
            ...
            db.run(
                ...
                function(err) {
                    ...
                    db.run(
                        ...
                        (err2) => {
                            if (err2) console.warn('[PAYMENT] Failed to log income transaction:', err2.message);
                            io.emit('db_updated', { module: 'credits' });
                            res.json({ success: true, message: `Payment of UGX ${paid.toLocaleString()} recorded for ${buyer_name}.` });
                        }
                    );
                }
            );
        ...
        if (token) {
            try {
                ...
                if (decoded.prefix && decoded.prefix !== 'public') {
                    ...
                    return db.all('SELECT setting_key, setting_value FROM app_settings', [], (err, rows) => {
                        if (err) return res.status(500).json({ error: err.message });
                        const settings = {};
...
app.get('/api/settings/logo', (req, res) => {
    ...
    db.get("SELECT setting_value FROM app_settings WHERE setting_key = 'company_logo'", [], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/settings/logo', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR' && req.user.role !== 'Admin') return res.status(403).json({ error: 'Forbidden' });
    const { logo_base64 } = req.body;
    
    db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('company_logo', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [logo_base64], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/settings/name', authenticateToken, (req, res) => {
    ...
    db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('business_name', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [business_name], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        emitAndBust('settings', 'settings');
...
app.post('/api/settings/details', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR' && req.user.role !== 'Admin') return res.status(403).json({ error: 'Forbidden' });
    const { business_name, business_location, business_contact, business_color, emp_prefix } = req.body;
    
    db.serialize(() => {
        if (business_name) {
            db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('business_name', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [business_name]);
        }
        db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('business_location', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [business_location]);
        if (business_color) {
            db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('business_color', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [business_color]);
        }
        if (emp_prefix) {
            db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('emp_prefix', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [emp_prefix]);
            db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('company_prefix', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [emp_prefix]);
        }
        db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('business_contact', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`, [business_contact], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            emitAndBust('settings', 'settings');
...
app.post('/api/shifts/toggle', authenticateToken, (req, res) => {
    ...
    if (!isAuthorized) {
        return res.status(403).json({ error: `Permission Denied: ${req.user.role}s cannot manage shifts.` });
    }
    ...
    if (action === 'OPEN') {
        db.run('INSERT INTO shifts (opened_by, status) VALUES (?, ?)', [user_id, 'OPEN'], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            // Reset all presence for a new shift
            db.run('UPDATE employees SET is_present = 0');
            io.emit('db_updated', { module: 'shift' });
            res.json({ message: 'Operational Shift is now OPEN. Presence state reset.' });
        });
    } else {
        db.run("UPDATE shifts SET closed_by = ?, close_time = CURRENT_TIMESTAMP, status = 'CLOSED' WHERE status = 'OPEN'", 
            [user_id], function(err) {
                if (err) return res.status(500).json({ error: err.message });
                io.emit('db_updated', { module: 'shift' });
                res.json({ message: 'Operational Shift is now CLOSED. Attendance logs suspended.' });
            }
        );
    }
});
...
app.patch('/api/users/:id/credentials', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (password && password.trim().length > 0) {
            ...
            sql = 'UPDATE employees SET password = ?, role = ?, username = ? WHERE id = ?';
...
app.patch('/api/users/:id/access', authenticateToken, (req, res) => {
    if (req.user.role !== 'HR' && req.user.role !== 'CEO') {
        return res.status(403).json({ error: 'Forbidden: Only HR can pause or restore account access.' });
    }
    const { id } = req.params;
    const { is_active } = req.body; // 0 = paused, 1 = active
    if (typeof is_active === 'undefined') {
        return res.status(400).json({ error: 'is_active field required (0 or 1).' });
    }
    db.run('UPDATE employees SET is_active = ? WHERE id = ?', [is_active ? 1 : 0, id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        if (this.changes === 0) return res.status(404).json({ error: 'Employee not found.' });
        emitAndBust('employees', 'employees');
        res.json({ message: is_active ? 'Account access restored.' : 'Account access paused.' });
    });
});
...
app.post('/api/users/tech-reset', authenticateToken, async (req, res) => {
    ...
    if (req.user.role !== 'CEO') return res.status(403).json({ error: 'Forbidden: System Technician access required.' });
    ...
    try {
        ...
        db.run('UPDATE employees SET password = ? WHERE username = ?', [hashedPassword, username], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            if (this.changes === 0) return res.status(404).json({ error: `User '${username}' not found.` });
...
app.post('/api/settings', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR') return res.status(401).json({ error: 'Unauthorized' });
    const { key, data } = req.body;
    
    if (key === 'business_modules' && req.user.name !== 'System Technician') {
        return res.status(403).json({ error: 'Access Denied: Only the System Technician can modify active business modules.' });
    }

    db.run('INSERT OR REPLACE INTO app_settings (setting_key, setting_value) VALUES (?, ?)', [key, JSON.stringify(data)], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/finance/summary', authenticateToken, (req, res) => {
    ...
    const weekAgo    = toEAT(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString());
    const monthStart = today.substring(0, 7) + '-01';

    db.all('SELECT amount, type, transaction_date FROM transactions', [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        ...
        const result = {
            ...
            allTime: { income: totalIncome,  expense: totalExpense,  profit: totalIncome  - totalExpense  },
            totalTransactions: rows.length,
            asOf: new Date(Date.now() + EAT_OFFSET_MS).toISOString().replace('T', ' ').substring(0, 16) + ' EAT',
            _debug: {
                serverToday: today,
...
app.post('/api/notes', authenticateToken, (req, res) => {
    ...
    if (!employee_id || !note_text) return res.status(400).json({ error: 'Employee ID and note text required' });

    db.run('INSERT INTO employee_notes (employee_id, note_text, note_type, created_by) VALUES (?, ?, ?, ?)',
...
app.delete('/api/notes/:id', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR') return res.status(403).json({ error: 'Forbidden' });
    db.run('DELETE FROM employee_notes WHERE id = ?', [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/employees/sick', authenticateToken, (req, res) => {
    const isStaffControl = ['CEO', 'HR', 'Supervisor', 'Manager'].includes(req.user.role);
    if (!isStaffControl) return res.status(403).json({ error: 'Permission Denied.' });

    const { employee_id, is_sick } = req.body;
    db.run('UPDATE employees SET is_sick = ?, is_present = 0 WHERE id = ?', 
        [is_sick ? 1 : 0, employee_id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            emitAndBust('employees', 'employees');
...
app.get('/api/attendance/summary', authenticateToken, (req, res) => {
    db.get(`SELECT 
        ...
        (SELECT COUNT(*) FROM employees WHERE is_sick = 1) as sick`, [], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json(row);
        }
    );
...
app.post('/api/scheduler/auto-generate', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR') return res.status(403).json({ error: 'HQ Only' });
    
    const { start_date, days_count, staff_per_shift = 2 } = req.body;
    
    db.all('SELECT id FROM employees WHERE is_active = 1 AND is_sick = 0', [], (err, emps) => {
        if (err) return res.status(500).json({ error: err.message });
        ...
        for (let i = 0; i < days_count; i++) {
            ...
            for (let shift of shifts) {
                ...
                for (let j = 0; j < staff_per_shift; j++) {
                    ...
                    queries.push([emps[globalEmpIndex % emps.length].id, dateStr, shift.start, shift.end]);
                    globalEmpIndex++;
                    assigned++;
                }
        ...
        if (queries.length === 0) {
            return res.json({ message: 'No active, healthy employees to schedule.' });
        }
        ...
        let hadError = false;
        queries.forEach(q => {
            db.run('INSERT INTO schedules (employee_id, shift_date, start_time, end_time) VALUES (?, ?, ?, ?)', q, (err) => {
                if (err) { hadError = true; console.error('[Scheduler] Insert error:', err.message); }
                processed++;
                if (processed === queries.length) {
                    io.emit('db_updated', { module: 'schedules' });
                    if (hadError) {
                        res.json({ message: `Shifts generated with some errors. ${processed} slots attempted.` });
                    } else {
                        res.json({ message: `Successfully generated ${queries.length} shift slots.` });
                    }
                }
            });
        });
...
app.post('/api/expense-categories', authenticateToken, (req, res) => {
    ...
    db.run('INSERT INTO expense_categories (name, budget_limit, color) VALUES (?, ?, ?)',
...
app.get('/api/alerts/low-stock', authenticateToken, (req, res) => {
    db.all('SELECT * FROM products WHERE stock < 10 ORDER BY stock ASC', [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
...
setInterval(() => {
    ...
    db.run(`UPDATE devices SET status = 'OFFLINE' 
            WHERE status = 'ONLINE' AND last_seen < datetime('now', '-90 seconds')`, function(err) {
        // NOTE: must use regular function (not arrow) so 'this' refers to sqlite stmt context
        if (err) { console.error('[RADAR] Device timeout error:', err.message); return; }
        if (this.changes > 0) {
            console.log(`[RADAR] Marked ${this.changes} inactive devices as OFFLINE`);
...
app.post('/api/devices/ping', (req, res) => {
    const { device_id, device_name, device_type, company_schema, client_ip } = req.body;
    // Determine the best IP: X-Forwarded-For > X-Real-IP > req.ip > socket remoteAddress
    let ip = req.headers['x-forwarded-for'] || req.headers['x-real-ip'] || req.ip || req.socket?.remoteAddress || '0.0.0.0';
    ...
    db.run(`INSERT INTO devices (device_id, device_name, device_type, ip_address, status, last_seen, company_schema) 
            VALUES (?, ?, ?, ?, 'ONLINE', datetime('now'), ?)
            ON CONFLICT(device_id) DO UPDATE SET 
            last_seen=datetime('now'), status='ONLINE', ip_address=?, company_schema=?`, 
            [device_id, device_name, device_type, ip, schema, ip, schema], (err) => {
        if (err) return res.status(500).json({ error: err.message });
        io.emit('db_updated', { module: 'devices' });
...
app.get('/api/devices', authenticateToken, (req, res) => {
    ...
    db.all('SELECT * FROM devices WHERE company_schema = ? ORDER BY last_seen DESC', [companySchema], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/devices/logout', authenticateToken, (req, res) => {
    ...
    db.run(`DELETE FROM devices WHERE device_id = ? AND company_schema = ?`, [device_id, companySchema], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        if (this.changes === 0) return res.status(404).json({ error: 'Device not found in your company.' });
...
app.post('/api/kiosk/auth', (req, res) => {
    ...
    const token = jwt.sign(
        { id: 0, role: 'Kiosk', name: device_name || 'Kiosk', permissions: {}, device_id },
        JWT_SECRET,
        { expiresIn: '24h' }
    );
    ...
    db.run(`INSERT INTO devices (device_id, device_name, device_type, ip_address, status, last_seen) 
            VALUES (?, ?, ?, ?, 'ONLINE', datetime('now'))
            ON CONFLICT(device_id) DO UPDATE SET 
            last_seen=datetime('now'), status='ONLINE', device_name=?, device_type=?, ip_address=?`,
            [device_id, device_name || 'Kiosk', device_type || 'Security Kiosk', ip, device_name, device_type, ip]);
...
app.post('/api/kiosk/scan', authenticateToken, (req, res) => {
    ...
    if (!identifier) return res.status(400).json({ error: 'Badge/code required.' });

    db.get('SELECT id, first_name, last_name, is_active, is_sick, is_present FROM employees WHERE id = ? OR employee_code = ?',
        [identifier, identifier], (err, emp) => {
        if (err) return res.status(500).json({ error: 'Database error' });
        if (!emp) return res.status(404).json({ error: 'Employee not found. Badge not recognized.' });
        if (emp.is_active === 0) return res.status(403).json({ error: 'ACCESS DENIED: This ID has been deactivated.' });
        if (emp.is_sick) return res.status(403).json({ error: 'ACCESS DENIED: Staff is registered as SICK/RECOVERING.' });

        const fullName = `${emp.first_name} ${emp.last_name}`;
        const localTime = new Date().toISOString();

        // Check shift status
        db.get('SELECT status FROM shifts ORDER BY id DESC LIMIT 1', (err, shift) => {
            if (shift && shift.status === 'CLOSED') {
                return res.status(403).json({ error: 'Business is currently CLOSED. Open a shift first.' });
            }
            ...
            if (emp.is_present === 1) {
                // CLOCK OUT
                db.run('INSERT INTO attendance_logs (employee_id, scan_time, scan_type, status) VALUES (?, ?, ?, ?)',
                    [emp.id, localTime, 'OUT', 'SUCCESS'], function(err) {
                    if (err) return res.status(500).json({ error: err.message });
                    db.run('UPDATE employees SET is_present = 0 WHERE id = ?', [emp.id]);
                    emitAndBust('attendance', 'attendance');
                    res.json({
                        message: `Hello Clocked OUT: ${fullName}`,
                        name: fullName, action: 'OUT',
                        device: device_id, time: localTime
                    });
                });
            } else {
                // CLOCK IN
                db.run('INSERT INTO attendance_logs (employee_id, scan_time, scan_type, status) VALUES (?, ?, ?, ?)',
                    [emp.id, localTime, 'IN', 'SUCCESS'], function(err) {
                    if (err) return res.status(500).json({ error: err.message });
                    db.run('UPDATE employees SET is_present = 1 WHERE id = ?', [emp.id]);
                    emitAndBust('attendance', 'attendance');
                    res.json({
                        message: `[OK] Clocked IN: ${fullName}`,
                        name: fullName, action: 'IN',
                        device: device_id, time: localTime
                    });
                });
            }
...
app.get('/api/calendar/events', authenticateToken, (req, res) => {
    ...
    let sql = 'SELECT * FROM calendar_events ORDER BY event_date ASC, start_time ASC';
    let params = [];
    if (month && year) {
        const monthStr = String(month).padStart(2, '0');
        sql = 'SELECT * FROM calendar_events WHERE event_date LIKE ? ORDER BY event_date ASC, start_time ASC';
...
app.post('/api/calendar/events', authenticateToken, (req, res) => {
    ...
    db.run(
        ...
        [title, description || '', event_date, start_time || '', end_time || '', event_type || 'Meeting', color || '#4F46E5', req.user.id],
        function(err) {
            if (err) return res.status(500).json({ error: err.message });
            io.emit('db_updated', { module: 'calendar' });
...
app.put('/api/calendar/events/:id', authenticateToken, (req, res) => {
    ...
    db.run(
        ...
        [title, description, event_date, start_time, end_time, event_type, color, req.params.id],
        function(err) {
            if (err) return res.status(500).json({ error: err.message });
            io.emit('db_updated', { module: 'calendar' });
...
app.post('/api/calendar/events/:id/minutes', authenticateToken, (req, res) => {
    ...
    db.run('UPDATE calendar_events SET minutes = ? WHERE id = ?', [minutes, req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        io.emit('db_updated', { module: 'calendar' });
        res.json({ success: true });
    });
...
try {
    ...
    console.log('[EMAIL] [OK] All email packages loaded (imap-simple, mailparser, nodemailer).');
} catch(e) {
    console.log('[EMAIL] [WARN] Required packages not installed:', e.message);
}

// Helper: Get email config from DB (promisified)
function getEmailConfig() {
    return new Promise((resolve, reject) => {
        db.get(`SELECT setting_value FROM app_settings WHERE setting_key = 'EMAIL_CONFIG'`, (err, row) => {
            if (err) return reject(new Error('Database error reading email config: ' + err.message));
            if (!row) return reject(new Error('Email not configured. Go to Secretary Hub â†’ Email Inbox â†’ [CONFIG] Config to set up your Gmail credentials.'));
            let cfg;
            try { cfg = JSON.parse(row.setting_value); } catch(e) { return reject(new Error('Email config is corrupted. Please re-enter your Gmail credentials.')); }
            if (!cfg.email) return reject(new Error('Gmail address is missing from config. Please update your email settings.'));
            if (!cfg.app_password) return reject(new Error('Gmail App Password is missing. Please update your email settings.'));
            resolve(cfg);
        });
    });
}
...
async function fetchGmailInbox() {
    if (!imapSimple || !mailSimpleParser) {
        throw new Error('Email packages not installed on server. Run: npm install imap-simple mailparser nodemailer');
    }
    ...
    let connection;
    try {
        console.log(`[EMAIL] Connecting to Gmail IMAP for ${cfg.email}...`);
        ...
        if (msg.includes('Invalid credentials') || msg.includes('AUTHENTICATIONFAILED')) {
            throw new Error('Gmail authentication failed. Your App Password may be incorrect or expired. Go to https://myaccount.google.com/apppasswords to generate a new one.');
        }
        if (msg.includes('ETIMEDOUT') || msg.includes('ENOTFOUND') || msg.includes('ECONNREFUSED')) {
            throw new Error('Cannot reach Gmail servers. Check your internet connection.');
        }
    ...
    try {
        ...
        let results = [];
        try {
            results = await connection.search([['SINCE', sinceDate]], { bodies: ['HEADER', ''], markSeen: false, struct: true });
        } catch (searchErr) {
            // Fallback: fetch ALL but limit to last 50
            console.warn('[EMAIL] SINCE search failed, falling back to ALL (limited):', searchErr.message);
            const allResults = await connection.search(['ALL'], { bodies: ['HEADER', ''], markSeen: false, struct: true });
        ...
        for (const item of latest) {
            ...
            try {
                ...
                const bodyPreview = (parsed.text || parsed.html?.replace(/<[^>]*>/g, '') || '').substring(0, 1000);
                const receivedAt = parsed.date ? parsed.date.toISOString() : new Date().toISOString();

                await new Promise((resolve) => {
                    db.run('INSERT OR IGNORE INTO email_messages (message_uid, from_address, from_name, subject, body_preview, received_at) VALUES (?, ?, ?, ?, ?, ?)',
                        [uid, fromAddr, fromName, subject, bodyPreview, receivedAt], function(err) {
        ...
        connection.end();
        if (newlySaved > 0) {
            console.log(`[EMAIL] [OK] Sync complete: ${newlySaved} new emails saved, ${totalProcessed} processed.`);
...
app.get('/api/emails', authenticateToken, (req, res) => {
    db.all('SELECT * FROM email_messages ORDER BY received_at DESC LIMIT 100', [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/emails/fetch', authenticateToken, async (req, res) => {
    if (!imapSimple) return res.status(500).json({ error: 'Email packages not installed on server.' });
    try {
...
app.post('/api/emails/send', authenticateToken, async (req, res) => {
    ...
    if (!allowedSend.includes(req.user.role)) return res.status(403).json({ error: `Permission denied. Only ${allowedSend.join(', ')} roles can send emails.` });

    const { to, subject, body } = req.body;
    if (!to || !subject || !body) return res.status(400).json({ error: 'Recipient, subject, and message body are all required.' });
    ...
    try {
        ...
        if (cfg.resend_api_key) {
            ...
            console.log(`[EMAIL] [OK] Sent via Resend to ${to}: "${subject}"`);
            ...
            const mailOptions = { from: `"${cfg.email}" <${cfg.email}>`, to, subject, text: body };
            const info = await transporter.sendMail(mailOptions);
            console.log(`[EMAIL] [OK] Sent via SMTP to ${to}: "${subject}"`);
        ...
        if (msg.includes('Invalid login') || msg.includes('auth')) {
            return res.status(500).json({ error: 'Gmail authentication failed. Your App Password may be wrong or expired. Re-enter it in [CONFIG] Config.' });
        }
...
app.post('/api/emails/blast', authenticateToken, async (req, res) => {
    ...
    if (!allowedBlast.includes(req.user.role)) return res.status(403).json({ error: 'Forbidden' });
    const { subject, body } = req.body;
    if (!subject || !body) return res.status(400).json({ error: 'Subject and message are required for the blast.' });

    try {
        // 1. Fetch all active emails
        const employees = await new Promise((resolve, reject) => {
            db.all('SELECT email FROM employees WHERE is_active = 1 AND email IS NOT NULL AND email != ""', [], (err, rows) => {
                if (err) return reject(err);
                resolve(rows || []);
            });
        });
        if (employees.length === 0) return res.status(400).json({ error: 'No active staff emails found in HR records.' });
        ...
        if (cfg.resend_api_key) {
            ...
            console.log(`[EMAIL] [OK] Blast sent via Resend to ${employees.length} staff`);
            res.json({ message: `Blast sent successfully to ${employees.length} workers!`, info: data });
        } else {
            const transporter = nodemailer.createTransport({
                host: 'smtp.gmail.com',
                port: 587,
            ...
            const info = await transporter.sendMail(mailOptions);
            console.log(`[EMAIL] [OK] Blast sent via SMTP to ${employees.length} staff`);
            res.json({ message: `Blast sent successfully to ${employees.length} workers!`, info });
        }
    } catch(err) {
        const msg = err.message || String(err);
...
app.patch('/api/emails/:id/read', authenticateToken, (req, res) => {
    db.run('UPDATE email_messages SET is_read = 1 WHERE id = ?', [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/settings/email', authenticateToken, (req, res) => {
    ...
    db.get(`SELECT setting_value FROM app_settings WHERE setting_key = 'EMAIL_CONFIG'`, (err, row) => {
        ...
        if (!finalEmail || (!finalPwd && !finalResend)) {
            return res.status(400).json({ error: 'Email and App Password (or Resend Key) are required.' });
        }

        db.run(`INSERT INTO app_settings (setting_key, setting_value) VALUES ('EMAIL_CONFIG', ?) ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`,
            [JSON.stringify({ email: finalEmail, app_password: finalPwd, resend_api_key: finalResend })], function(err) {
                if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/settings/email', authenticateToken, (req, res) => {
    db.get(`SELECT setting_value FROM app_settings WHERE setting_key = 'EMAIL_CONFIG'`, (err, row) => {
        ...
        try {
            ...
            res.json({ configured: true, email: c.email, has_password: !!c.app_password });
...
app.post('/api/messages', authenticateToken, (req, res) => {
    ...
    if (!to_id || !content) return res.status(400).json({ error: 'Recipient and message required' });
    db.run('INSERT INTO internal_messages (from_id, to_id, subject, content) VALUES (?, ?, ?, ?)',
        [req.user.id, to_id, subject || '', content], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            io.emit('db_updated', { module: 'messages' });
...
app.patch('/api/messages/:id/read', authenticateToken, (req, res) => {
    db.run('UPDATE internal_messages SET is_read = 1 WHERE id = ? AND to_id = ?',
...
app.get('/api/messages/unread', authenticateToken, (req, res) => {
    db.get('SELECT COUNT(*) as count FROM internal_messages WHERE to_id = ? AND is_read = 0',
        [req.user.id], (err, row) => {
            if (err) return res.status(500).json({ error: err.message });
...
db.run(`INSERT OR IGNORE INTO system_meta (key, value) VALUES ('version', '1.0 Genesis')`);
...
app.get('/api/system-meta/:key', authenticateToken, (req, res) => {
    db.get('SELECT * FROM system_meta WHERE key = ?', [req.params.key], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!row) return res.status(404).json({ error: 'Not found' });
        res.json(row);
    });
...
app.delete('/api/system-meta/:key', authenticateToken, (req, res) => {
    db.run('DELETE FROM system_meta WHERE key = ?', [req.params.key], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/shift-assignments', authenticateToken, (req, res) => {
    ...
    if (!allowed.includes(req.user.role)) return res.status(403).json({ error: 'Permission denied.' });
    const { employee_id, shift_date, slot, start_time, end_time } = req.body;
    if (!employee_id || !shift_date || !slot || !start_time || !end_time) {
        return res.status(400).json({ error: 'employee_id, shift_date, slot, start_time, end_time are required.' });
    }
    db.run(
...
app.patch('/api/shift-assignments/:id', authenticateToken, (req, res) => {
    ...
    db.run(
        ...
        [employee_id, shift_date, slot, start_time, end_time, req.user.id, req.params.id],
        function(err) {
            if (err) return res.status(500).json({ error: err.message });
...
app.delete('/api/shift-assignments/:id', authenticateToken, (req, res) => {
    if (!['CEO', 'HR'].includes(req.user.role)) return res.status(403).json({ error: 'Permission denied.' });
    db.run('DELETE FROM shift_assignments WHERE id = ?', [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        io.emit('db_updated', { module: 'shift_assignments' });
...
app.delete('/api/shift-assignments/week/:weekStart', authenticateToken, (req, res) => {
    if (!['CEO', 'HR'].includes(req.user.role)) return res.status(403).json({ error: 'Permission denied.' });
    ...
    db.run('DELETE FROM shift_assignments WHERE shift_date BETWEEN ? AND ?', [from, to], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        io.emit('db_updated', { module: 'shift_assignments' });
        res.json({ message: `Cleared ${this.changes} assignments for ${from} â€“ ${to}.` });
    });
});
...
app.get('/api/scheduler/coverage', authenticateToken, (req, res) => {
    if (!['CEO', 'HR', 'Supervisor'].includes(req.user.role)) return res.status(403).json({ error: 'Forbidden' });
    const { from, to } = req.query;
    if (!from || !to) return res.status(400).json({ error: 'from and to required.' });

    db.get('SELECT setting_value FROM app_settings WHERE setting_key = ?', ['BUSINESS_BLUEPRINT'], (err, row) => {
        const blueprint = row ? (() => { try { return JSON.parse(row.setting_value); } catch(e){ return null; }})() : null;
        db.all(`
...
app.post('/api/scheduler/smart-generate', authenticateToken, (req, res) => {
    if (!['CEO', 'HR'].includes(req.user.role)) return res.status(403).json({ error: 'HQ Only.' });
    const { week_start, clear_existing = true } = req.body;
    if (!week_start) return res.status(400).json({ error: 'week_start (YYYY-MM-DD) required.' });

    // --- 1. Load Business Blueprint ---
    db.get('SELECT setting_value FROM app_settings WHERE setting_key = ?', ['BUSINESS_BLUEPRINT'], (bpErr, bpRow) => {
        const blueprint = bpRow ? (() => { try { return JSON.parse(bpRow.setting_value); } catch(e){ return null; }})() : null;

        // Blueprint defaults if not configured
        const opDays     = blueprint?.days || ['Mon','Tue','Wed','Thu','Fri','Sat'];
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if (opDayNums.has(dayNum)) weekDates.push(d.toISOString().split('T')[0]);
        }

        if (weekDates.length === 0) {
            return res.json({ message: 'No operational days in this week per Business Blueprint.', generated: 0 });
        }

        // --- 2. Load employees grouped by role ---
        db.all('SELECT id, first_name, last_name, role FROM employees WHERE is_active = 1 AND is_sick = 0 ORDER BY role, last_name', [], (empErr, employees) => {
            if (empErr) return res.status(500).json({ error: empErr.message });
            if (employees.length === 0) return res.json({ message: 'No active employees to schedule.', generated: 0 });
            ...
            const doGenerate = () => {
                ...
                db.all(`SELECT employee_id, COUNT(*) as cnt FROM shift_assignments WHERE shift_date BETWEEN ? AND ? GROUP BY employee_id`,
                    [fromDate, toDate], (fcErr, fcRows) => {

                    const fcMap = {};
                    (fcRows || []).forEach(r => fcMap[r.employee_id] = r.cnt);
                    // Apply to employee pools
                    Object.values(byRole).forEach(pool => pool.forEach(e => { e.shiftCount = fcMap[e.id] || 0; }));
                    ...
                    for (const date of weekDates) {
                        for (const slot of slots) {
                            for (const [role, required] of Object.entries(staffingReq)) {
                                const needed = parseInt(required, 10) || 0;
                                if (needed <= 0) continue;
                                const pool = byRole[role] || [];
                                if (pool.length === 0) {
                                    gaps.push({ date, slot: slot.name, role, needed, available: 0 });
                                    continue;
                                }
                                ...
                                if (picked.length < needed) {
                                    gaps.push({ date, slot: slot.name, role, needed, available: pool.length });
                                }
                                picked.forEach(emp => {
                                    insertRows.push([emp.id, date, slot.name, slot.start, slot.end, null]);
                            ...
                            if (Object.keys(staffingReq).length === 0) {
                                employees.sort((a, b) => (fcMap[a.id] || 0) - (fcMap[b.id] || 0));
                    ...
                    if (insertRows.length === 0) {
                        return res.json({ message: 'Nothing to schedule. Check Blueprint staffing requirements.', generated: 0, gaps });
                    }

                    // --- 5. Batch insert ---
                    let done = 0, hadError = false;
                    insertRows.forEach(row => {
                        db.run('INSERT INTO shift_assignments (employee_id, shift_date, slot, start_time, end_time, assigned_by) VALUES (?,?,?,?,?,?)', row, (iErr) => {
                            if (iErr) { hadError = true; console.error('[SmartScheduler] Insert error:', iErr.message); }
                            done++;
            ...
            if (clear_existing) {
                db.run('DELETE FROM shift_assignments WHERE shift_date BETWEEN ? AND ?', [fromDate, toDate], doGenerate);
            } else {
                doGenerate();
            }
        });
...
app.get('/api/payroll/status', authenticateToken, (req, res) => {
    ...
    `, [monthYear, monthYear], (err, rows) => {
        ...
        rows.forEach(emp => {
            ...
            let months = (selDate.getFullYear() - createdDate.getFullYear()) * 12 + (selDate.getMonth() - createdDate.getMonth()) + 1;
            ...
            let arrears = (totalExpected - emp.salary) - (totalPaidAllTime - paidThisMonth);
...
app.post('/api/payroll', authenticateToken, (req, res) => {
    ...
    db.get('SELECT first_name, last_name FROM employees WHERE id = ?', [employee_id], (err, emp) => {
        ...
        db.run('INSERT INTO transactions (amount, type, description, recorded_by, payment_status) VALUES (?, ?, ?, ?, ?)', 
        [amount, 'EXPENSE', desc, req.user.id, 'PAID'], function(err) {
            if (err) return res.status(500).json({ error: 'Failed to record expense transaction' });
            
            const txId = this.lastID;
            
            // Insert into payroll_records
            db.run('INSERT INTO payroll_records (employee_id, month_year, amount, transaction_id) VALUES (?, ?, ?, ?)',
            [employee_id, month_year, amount, txId], function(err) {
                if (err) return res.status(500).json({ error: 'Failed to record payroll' });
                
                emitAndBust('transactions', 'finance_summary');
                res.json({ success: true, message: 'Payroll processed', record_id: this.lastID, transaction_id: txId });
            });
        });
    });
});
...
app.get('/api/notices', authenticateToken, (req, res) => {
    db.all('SELECT * FROM notices ORDER BY created_at DESC LIMIT 50', [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.delete('/api/notices/:id', authenticateToken, (req, res) => {
    if (req.user.role !== 'CEO' && req.user.role !== 'HR') {
        return res.status(403).json({ error: 'Forbidden: Only administrators can delete notices.' });
    }
...
app.post('/api/pos_orders/:id/cod-received', authenticateToken, (req, res) => {
    db.serialize(() => {
        ...
        db.get('SELECT id as pos_order_id, transaction_id, total_amount FROM pos_orders WHERE id = ?', [req.params.id], (err, row) => {
            if (err || !row) {
                db.run('ROLLBACK');
            ...
            db.run('UPDATE deliveries SET status = ? WHERE order_id = ?', ['Delivered', row.pos_order_id], function(err) {
                if(err) { db.run('ROLLBACK'); return res.status(500).json({ error: err.message }); }
                
                db.run('UPDATE transactions SET payment_status = ? WHERE id = ?', ['PAID', row.transaction_id], function(err) {
                    if(err) { db.run('ROLLBACK'); return res.status(500).json({ error: err.message }); }
                    
                    db.run('UPDATE pos_orders SET amount_paid = ? WHERE id = ?', [row.total_amount, row.pos_order_id], function(err) {
                        if(err) { db.run('ROLLBACK'); return res.status(500).json({ error: err.message }); }
                        
                        db.run('COMMIT', (err) => {
                            if (err) { db.run('ROLLBACK'); return res.status(500).json({ error: 'Commit failed' }); }
                            emitAndBust('transactions', 'finance_summary');
                            io.emit('db_updated', { module: 'pos' });
                            io.emit('db_updated', { module: 'deliveries' });
...
app.get('/api/deliveries', authenticateToken, (req, res) => {
    db.all(`SELECT d.*, p.transaction_id as receipt_number FROM deliveries d LEFT JOIN pos_orders p ON d.order_id = p.id ORDER BY d.created_at DESC`, [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        if (req.user.role === 'Driver') {
            const filtered = rows.filter(r => !r.driver_id || r.driver_id === req.user.id);
            res.json(filtered);
        } else {
            res.json(rows);
        }
...
app.get('/api/employees/upcoming-pay', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT id, first_name, last_name, role, department, salary, next_pay_date FROM employees WHERE is_active = 1 AND next_pay_date IS NOT NULL`, [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/deliveries', authenticateToken, (req, res) => {
    const { client_name, client_phone, client_location, driver, items, fee, status, notes } = req.body;
    if (!client_name || !client_phone || !client_location) return res.status(400).json({ error: 'Client name, phone and location required.' });
    db.run(`INSERT INTO deliveries (client_name, client_phone, client_location, status, order_id, created_at)
        ...
        function(err) {
            ...
            db.run(`UPDATE deliveries SET order_id=? WHERE id=?`, [this.lastID, this.lastID]);
            res.json({ id: this.lastID, success: true });
        });
});

app.post('/api/deliveries/:id/claim', authenticateToken, (req, res) => {
    if (req.user.role !== 'Driver') return res.status(403).json({ error: 'Only drivers can claim jobs.' });
    db.get(`SELECT driver_id FROM deliveries WHERE id = ?`, [req.params.id], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!row) return res.status(404).json({ error: 'Delivery not found.' });
        if (row.driver_id) return res.status(400).json({ error: 'Job already claimed.' });
        
        db.run(`UPDATE deliveries SET driver_id = ?, driver_name = ? WHERE id = ?`, 
            [req.user.id, req.user.name, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true });
        });
    });
...
app.get('/api/petty-cash', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM petty_cash WHERE shift_id = ? ORDER BY created_at DESC`, [shiftId], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
    });
});
...
app.post('/api/petty-cash', authenticateToken, (req, res) => {
    ...
    if (!shift_id || !purpose || !amount) return res.status(400).json({ error: 'Missing fields' });
    db.run(`INSERT INTO petty_cash (shift_id, purpose, amount, recorded_by) VALUES (?, ?, ?, ?)`,
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    seedDemoTenant(db, db.asyncLocalStorage).catch(err => console.error('[SEED] Failed to seed demo tenant:', err));
    ...
    asyncLocalStorage.run('public', () => {
        ...
        if (require('./database').dbPath) { // simple check if SQLite
            execSql = execSql.replace(/SERIAL PRIMARY KEY/g, 'INTEGER PRIMARY KEY AUTOINCREMENT').replace(/TIMESTAMP/g, 'DATETIME');
        }
        db.run(execSql, [], (err) => {
            if (err) console.error("Error creating tech_users table:", err.message);
            else {
                db.get("SELECT COUNT(*) as count FROM tech_users", async (err, row) => {
                    if (!err && row && row.count === 0) {
                        const defaultHash = await bcrypt.hash('Jomish9!!', 10);
                        db.run("INSERT INTO tech_users (username, password) VALUES (?, ?)", ['tech', defaultHash]);
                    }
                });
            }
        });
    ...
    for (const name of Object.keys(interfaces)) {
        for (const net of interfaces[name]) {
            if (net.family === 'IPv4' && !net.internal) {
                console.log(`REMOTE ACCESS: http://${net.address}:${PORT}`);
            }
        }
    }
    console.log(`HOSTNAME:      http://${os.hostname()}.local:${PORT}`);
    console.log(`-----------------------------------------\n`);
    ...
    db.get('SELECT status FROM shifts ORDER BY id DESC LIMIT 1', (err, row) => {
        if (!err && (!row || row.status === 'CLOSED')) {
            db.run('INSERT INTO shifts (opened_by, status) VALUES (0, ?)', ['OPEN'], (err) => {
                if (!err) {
                    db.run('UPDATE employees SET is_present = 0');
                    console.log('[AUTO-SHIFT] Operational shift automatically opened at startup.');
                }
            });
        }
    });
    ...
    console.log(`Service advertised as ${config.mDNS_name || 'business-system'}.local`);
}).on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
        console.error(`Port ${PORT} is already in use! Please close other server windows or restart using the .bat file.`);
        process.exit(1);
    } else {
