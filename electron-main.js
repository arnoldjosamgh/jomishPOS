const { app, BrowserWindow, ipcMain, shell, session, globalShortcut, clipboard, dialog } = require('electron');
...
function createWindow() {
    ...
    const configureSession = (sess) => {
        ...
        sess.webRequest.onBeforeSendHeaders((details, callback) => {
            // Only force no-cache for local app assets
    ...
    waitForServer('http://localhost:3005/api/discover').then((ready) => {
        ...
        mainWindow.loadURL('http://localhost:3005/login.html');
...
app.on('ready', () => {
    // Clear ALL Electron caches on every startup so we always load fresh JS/HTML from disk
    session.defaultSession.clearCache()
        .then(() => session.defaultSession.clearStorageData({ storages: ['cachestorage', 'serviceworkers'] }))
        .catch(() => {})
